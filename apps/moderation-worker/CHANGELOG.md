...
- **Ban-confirmation embed says "canceled"** (`src/handlers/buttons/ban-confirmation.ts`), American
  spelling per `.agents/skills/audit-shared/american-english.md`.
...
- **Migration `0013_moderation_log_user_actions.sql` must be hand-run on `xivdyetools-presets` BEFORE this version is deployed** — i.e. **before merging `security-audit-2026-08-29`**, because `deploy-moderation-worker.yml` runs on push to `main` and the merge itself auto-deploys 1.6.0. The migration is safe against what is deployed today (moderation-worker 1.5.0 writes nothing to `moderation_log`; presets-api 2.2.0 only ever binds a real `preset_id`), so run it now rather than trying to time it against the merge. From `apps/presets-api`, `wrangler d1 execute xivdyetools-presets --remote --file=migrations/0013_moderation_log_user_actions.sql` (verification queries in the file header and in `docs/operations/POST_MERGE_CHECKLIST.md`). Deployed early / applied late, **every ban and unban fails until the migration lands**: the audit rows share the ban's atomic batch, so the missing `target_discord_id` column aborts the whole batch — nothing is lost and there is no half state (no `banned_users` row, no hidden presets), and the moderator sees "Ban system schema is out of date — apply presets-api migration 0013." with the raw D1 message in the logs. Applying the migration fixes it with no redeploy.
- **Production refuses every request — `/health` included — while `RL_COMMAND` or `RL_AUTOCOMPLETE` is unbound** (FINDING-013, see the Security section below for the full story). This version's `wrangler.toml` already binds both in `[env.production]`, so a normal `deploy:production` needs no extra step; the callout here is that this is now a hard outage, not a silent degrade, if a future config change ever drops one.
...
- **Ban, unban, hide and restore are written to `moderation_log`** (docs/audits/2026-08-29-security, FINDING-018 — the half of FINDING-034 that 1.5.0 deferred as "a presets-api-owned decision"). A ban wrote a `banned_users` row and flipped the author's `approved` presets to `hidden` without leaving a single audit row: `GET /api/v1/moderation/:presetId/history` could not explain why a preset had vanished from the gallery, `/moderation/stats` never counted a ban, and the only accountability trail was `banned_users` itself. presets-api owns that table and its `preset_id` was `NOT NULL`, so a user-level action could not be recorded there at all.
  ...
  Migration `0013_moderation_log_user_actions.sql` (presets-api, hand-run — see Deploy notes) makes `preset_id` nullable and adds `target_discord_id`, and `banUser` / `unbanUser` now write the rows themselves, in the **same** `db.batch()` as the statements they describe: one `ban` / `unban` row per user action (`preset_id` NULL, `target_discord_id` = the banned user) plus one `hide` / `restore` row per preset the UPDATE actually flips, generated by an `INSERT … SELECT` over the same `author_discord_id` / `status` predicate placed just before that UPDATE. A D1 batch is atomic, so the trail cannot drift from the effect — a ban that logs nothing is impossible, and so is a `ban` row for a ban that did not happen (a second moderator racing the same ban loses on `idx_banned_users_discord_active`, which aborts the whole batch). Atomicity alone does not say the same about an unban, because closing an already-closed ban row is not an error, just an UPDATE that matches nothing: the `unban` row is therefore written by an `INSERT … SELECT … WHERE changes() > 0` placed immediately after that UPDATE — the idiom presets-api uses for its own status-change rows — so a moderator who loses the race commits no row. The `hide` / `restore` rows are conditional by construction: they select exactly the presets their UPDATE is about to flip, so a race that leaves nothing to flip logs nothing. Ids are UUID v4 (`crypto.randomUUID()` for the user-level row; the per-preset ids come from SQLite's `randomblob`/`hex` recipe with the version and variant nibbles pinned, since one statement writes many rows). Rows carry Discord ids, the moderator's typed reason and a timestamp — **never a username**; `unban` and `restore` rows carry a NULL reason because the unban command takes none. No new binding, no presets-api code change, no presets-api deploy. A ban attempted against a database that has not had 0013 applied still fails loudly, but now says so usefully: the D1 "has no column named" error maps to the channel-safe **"Ban system schema is out of date — apply presets-api migration 0013."** instead of the generic "Failed to ban user.", naming the fix without naming a table or column (MOD-8); the raw message still reaches the structured log through `cause`. The two exported single-statement helpers behind those UPDATEs (`hideUserPresets` / `restoreUserPresets`) are not on this path — `banUser` / `unbanUser` batch the lower-level statement forms directly — and now carry a `@remarks` warning that any future caller must batch the same log statement ahead of the UPDATE, since they are the only status-flipping paths left without one.

- **`tests/wrangler-config.test.ts` pins the `wrangler.toml` invariants nothing else in this worker tests** (FINDING-023). `workers_dev = false` in both envs, no `routes` on the dev worker, the two production custom domains, `ENVIRONMENT` per env, the `KV` namespace shared with discord-worker's *production* namespace (deliberately identical in both of this worker's own envs too — it has no KV of its own), the `DB` database shared with presets-api (same id, cross-checked against `apps/presets-api/wrangler.toml`), the `PRESETS_API` service binding, the four `RL_COMMAND` / `RL_AUTOCOMPLETE` `namespace_id`s (1033/1034 dev, 1031/1032 production) and the absence of an `[env.preview]` block — none of it reachable from a source test, all of it invisible until a bad deploy. `vitest.config.ts`'s `include` gained `tests/**/*.test.ts` (it was `src/**/*.test.ts` only, so a file placed there would silently never have run) and `tsconfig.json` picked up the same directory so `type-check` compiles it too.

### Security

Four small hardening fixes from `docs/audits/2026-08-29-security`, each already landed on a sibling worker.

- **The ban log line no longer carries the banned user's display name or the moderator's typed reason** (FINDING-011). `logger.info('User banned', …)` put a Discord username and free-text commentary *about a person* into a structured log that nothing promises to retain or protect, on the one code path where that text is at its most sensitive. It now logs `{ targetUserId, moderatorId, presetsHidden, reasonLength }` — ids, a count and a length. Nothing is lost: the accountability copy of the reason lives in `moderation_log` (see above) and in the moderation-channel post, which are the records of record. A sweep of every other `logger.*` call in this worker found no second site logging a name, reason or preset name — though seven sites still log the raw `customId` verbatim for routing/diagnostics (`index.ts`'s "Handling component" / "Handling modal" and both "Unable to identify user…" lines, its "Rate limit exceeded" warn for buttons and modals, plus the malformed-id warnings in `ban-confirmation.ts` and `ban-reason.ts`), and a `ban_confirm_…` / `ban_reason_modal_…` id issued before the 1.5.0 fix (FINDING-007) can still carry a base64url-encoded display name inside that string. No live path produces one today — only an old in-flight button or modal posted before that deploy could still carry one if clicked.
