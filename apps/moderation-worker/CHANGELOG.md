...
- **Production refuses to start clean without the two rate-limit bindings** (FINDING-013). 1.5.0 moved per-user limiting onto `RL_COMMAND` / `RL_AUTOCOMPLETE`, but nothing checked they were bound: a config edit or dashboard change that dropped one degraded in silence to the KV limiter, which cannot throttle a fast client. `validateEnv` now reports `Missing required env var in production: RL_COMMAND` / `…: RL_AUTOCOMPLETE` when `ENVIRONMENT === 'production'`, mirroring presets-api and oauth; both stay optional in development and tests so the KV fallback still works there. `wrangler.toml` gained `ENVIRONMENT` in **both** blocks (`"development"` top-level, `"production"` under `[env.production.vars]` — vars are not inheritable, and a production deploy without the line would skip the check). Startup-validation failures are still logged and the request proceeds, as before — no behaviour change beyond the new error text. Side effect on the routeless dev worker: `ENVIRONMENT = "development"` also switches on the `isDev` arm of the global error handler, so a 500 there now includes the message and stack. That worker has no routes and `workers_dev = false`, so only a `wrangler dev` session ever sees it; production is unaffected.
