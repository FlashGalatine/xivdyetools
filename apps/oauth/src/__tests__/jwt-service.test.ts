...
const mintToken = (
  env: Env,
  options: { global_name?: string | null; avatar?: string | null } = {
    global_name: 'Test User',
    avatar: 'abc123hash',
  },
...
describe('JWT Service', () => {
  ...
  describe('signJwtData / verifyJwtData (DEAD-019 HMAC adoption)', () => {
    // Pinned vector: computed with the OLD hand-rolled implementation
    // (crypto.subtle.importKey({name:'HMAC',hash:'SHA-256'}) + sign +
    // base64url-encode, no key-length check) BEFORE it was replaced with
    // @xivdyetools/auth's hmacSign/hmacVerify. Re-running the same inputs
    // through the new delegation must reproduce this exact string —
    // that equivalence is the adoption's safety proof.
    const PINNED_SECRET = 'pinned-vector-secret-for-dead-019-test!';
    const PINNED_DATA = 'DEAD-019 pinned vector: header.payload';
    const PINNED_SIGNATURE = 'k-PyzXCM42BdFEHkzV3fXFdWNc3J1FU8rOeylbsvO0w';

    it('signJwtData reproduces the pre-adoption signature for a fixed vector', async () => {
      const signature = await signJwtData(PINNED_DATA, PINNED_SECRET);
      expect(signature).toBe(PINNED_SIGNATURE);
    });

    it('verifyJwtData accepts the pinned signature', async () => {
      const isValid = await verifyJwtData(PINNED_DATA, PINNED_SIGNATURE, PINNED_SECRET);
    ...
    it('verifyJwtData rejects a tampered signature', async () => {
      ...
      const isValid = await verifyJwtData(PINNED_DATA, tampered, PINNED_SECRET);
      expect(isValid).toBe(false);
    });

    it('round-trips arbitrary data through signJwtData/verifyJwtData', async () => {
      const signature = await signJwtData('round-trip-payload', PINNED_SECRET);
      expect(await verifyJwtData('round-trip-payload', signature, PINNED_SECRET)).toBe(true);
    });
  });
  ...
  describe('verifyJWTWithRevocationCheck', () => {
    ...
    it('should throw for revoked token', async () => {
      ...
      await expect(verifyJWTWithRevocationCheck(token, mockEnv.JWT_SECRET, kv)).rejects.toThrow(
    ...
    it('should throw for invalid signature', async () => {
      ...
      await expect(verifyJWTWithRevocationCheck(token, wrongSecret, kv)).rejects.toThrow(
        'Invalid JWT',
      );
    });
