...
- **BUG-042 — dye-info test asserted only embed presence.** `src/commands/info.test.ts` now asserts actual embed content for a known dye: title matches dye name, color matches hex, and description contains the comparison URL. Previously it could not catch incorrect field values.
