...
describe('PresetsHandler', () => {
  ...
  describe('GET /api/v1/presets', () => {
    ...
    it('matches a secondary category, not just the primary', async () => {
      ...
      const zonesBinds = mockDb._bindings.flat().filter((b) => b === 'zones');
  ...
  describe('PATCH /api/v1/presets/refresh-author', () => {
    it('should require authentication', async () => {
      const res = await app.request('/api/v1/presets/refresh-author', { method: 'PATCH' }, env);
  ...
  describe('POST /api/v1/presets', () => {
    ...
    describe('dead-letter retention (FINDING-017)', () => {
      ...
      function setupSubmissionMock(extra: (query: string) => unknown = () => undefined): void {
        mockDb._setupMock((query: string) => {
      ...
      it('prunes aged-out dead letters off the response path', async () => {
        ...
        try {
          ...
          expect(res.status).toBe(201);
          expect(mockDb._queries.some((q) => /DELETE FROM failed_notifications/i.test(q))).toBe(
            false,
          );
          ...
          expect(deletes.find((d) => /resolved_at IS NOT NULL/i.test(d.query))?.bindings).toEqual([
            '2026-07-30 12:00:00',
          ]);
          expect(deletes.find((d) => /resolved_at IS NULL/i.test(d.query))?.bindings).toEqual([
            '2026-05-31 12:00:00',
          ]);
        } finally {
          vi.useRealTimers();
      ...
      it('answers 201 even when the prune throws', async () => {
        ...
        expect(mockDb._queries.some((q) => /DELETE FROM failed_notifications/i.test(q))).toBe(true);
      });
    });
    ...
    const duplicateMock = (existing: ReturnType<typeof createMockPresetRow>) => (query: string) => {
      if (query.includes('FROM categories')) {
        return [{ id: 'aesthetics' }, { id: 'jobs' }];
      }
      if (query.includes('COUNT') && query.includes('author_discord_id')) {
  ...
  describe('POST /api/v1/presets/:id/preview-image', () => {
    ...
    it('should store the image, mark it pending, and delete a previous image', async () => {
      ...
      expect(mockDb._queries.some((q) => q.includes("preview_image_status = 'pending'"))).toBe(
        true,
      );
    });
    ...
    it('records a dead-letter row when the Discord notification is rejected', async () => {
      ...
      await Promise.allSettled(waitUntilPromises);
      expect(mockDb._queries.some((q) => q.includes('INSERT INTO failed_notifications'))).toBe(
        true,
      );
    });
  });
  ...
  describe('DELETE /api/v1/presets/:id', () => {
    it('should require authentication', async () => {
      const res = await app.request('/api/v1/presets/preset-123', { method: 'DELETE' }, env);
    ...
    it('should bind the correct preset ID to both cascade delete queries', async () => {
      ...
      const presetIdBindings = mockDb._bindings.filter((b) => b.includes(presetId));
    ...
    it('should execute delete operations in correct order (votes first, then preset)', async () => {
      ...
      const voteDeleteIndex = mockDb._queries.findIndex((q) => q.includes('DELETE FROM votes'));
  ...
  describe('PATCH /api/v1/presets/:id', () => {
    ...
    it("omits the duplicate summary from the 409 when the colliding preset is pending and not the caller's", async () => {
      const target = createMockPresetRow({
        id: 'preset-123',
        author_discord_id: '123',
        status: 'approved',
      });
    ...
    it('keeps the {id, name, author_name} duplicate summary in the 409 when the colliding preset is approved', async () => {
      ...
      expect(res.status).toBe(409);
      const body = (await res.json()) as {
        duplicate?: { id: string; name: string; author_name: string | null };
      };
