...
- `checkBanStatus` and `requireNotBannedCheck` (`middleware/ban-check.ts`, DEAD-011) — the two
  ...
  `banLookupFailure` helpers and their fail-closed behavior are untouched.
...
Security audit remediation (docs/audits/2026-08-29-security, Sprint 1: FINDING-004 / 005 / 006 / 010 / 011 / 013 / 015 / 016 / 017 / 023). Minor bump: anonymous responses lose `author_discord_id` and gain nothing; authenticated (web) responses gain `is_owner`; the v1 bot signature (`X-Request-Signature`) is no longer accepted — only `X-Request-Signature-V2`; a new `text_edit` daily cap applies before every name/description edit is moderated; content moderation now fails closed when Perspective cannot answer. **Apply migration `0012_submission_events_text_edit.sql` before deploying; at worst right after — the text-edit cap is inert until it is applied, and nothing else depends on it.** See Deploy notes.
...
- **⚠️ Response-shape change: `author_discord_id` is no longer published to anonymous callers** (docs/audits/2026-08-29-security, FINDING-016). `rowToPreset()` put the author's Discord snowflake on every serialized preset, and the gallery's listing, `?search=` / `?category=` filtering, `/featured` and `GET /presets/:id` are all anonymous routes — so any visitor (or scraper) of `api.xivdyetools.app` collected the Discord id of every author who had ever submitted a preset, ready for cross-referencing and unsolicited DMs. Only privileged consumers ever needed it: the web app's edit form uses it for its owner check on an authenticated page, and the bots use it behind HMAC. The privacy guide promises the display name, not the id.

  One serializer now stands between a preset and an HTTP client — `toPublicPreset(preset, viewer)` in `services/preset-service.ts`, applied by every route in `handlers/presets.ts` that returns a preset (list, `/featured`, `/mine`, `GET /:id`, the `POST` 201 and its duplicate answers, the `PATCH` 200). An **anonymous** response carries no `author_discord_id` key at all and no `is_owner` (`author_name` is unchanged and remains the author identity the gallery shows). A **web (JWT)** response gains `is_owner: boolean` and carries `author_discord_id` only when the preset is the caller's own or the caller is a moderator (`MODERATOR_IDS`). **Bot (HMAC)** responses and the moderator-only routes under `/api/v1/moderation/*` are untouched — discord-worker and moderation-worker run their own owner checks on the id — as are the server-to-bot `notifyDiscordBot` payloads. No client change is required: the owner still receives their own id, so `preset.author_discord_id !== user.id` in the web app's edit form keeps resolving correctly (a non-owner now compares `undefined`, which is still "not the owner"); `is_owner` is the field new client code should read.

- **⚠️ Response-shape change: dead-letter rows keep the preset id, not the notification; both retention tables are pruned by age and cascade on delete** (docs/audits/2026-08-29-security, FINDING-017). `storeFailedNotification()` stored `JSON.stringify(payload)` — the author's Discord snowflake, their display name and the whole preset (name, description, dyes, tags) — every time a moderation embed exhausted its retries. No `DELETE` against `failed_notifications` or `submission_events` existed anywhere in the worker, so those rows outlived the preset, the author's deletion request and the moderator who resolved them, and the bot's privacy policy had no retention line for either table.

  The dead-letter row now holds only `{ type, preset_id, moderation_status? }` — produced by the single writer `toDeadLetterRecord()`, so a future call site cannot re-introduce the content — while `error`, `attempts`, `created_at` and `resolved_at` (everything triage actually uses) are unchanged. Both tables are pruned by age, since this worker has no cron trigger. Dead letters: resolved rows older than `FAILED_NOTIFICATION_RESOLVED_RETENTION_DAYS` = 30, unresolved rows older than `FAILED_NOTIFICATION_UNRESOLVED_RETENTION_DAYS` = 90 — swept from four places, because the dead-letter *write* is by design rare (a notification has to exhaust every retry) and the window above is a promise the policy now makes: `storeFailedNotification()`, both moderator paths (`GET /moderation/failed-notifications` and the resolve `PATCH`), and every `POST /api/v1/presets` via `waitUntil`, off the response path. `submission_events`: rows older than `SUBMISSION_EVENT_RETENTION_DAYS` = 30, on `recordSubmissionEvent()`. The prunes are age-based only — never narrowed by user or kind, and 30 days outside the UTC day every daily cap counts — so FINDING-008's rule that no user action deletes a quota row still holds. A prune runs **before**, and never inside, the `db.batch` that writes the row it is making space for: a D1 batch is atomic, and housekeeping must not be able to discard the dead letter (or the append-only quota row) it was tidying up for. Failures are swallowed and logged at `warn` with a count and nothing else. `DELETE /api/v1/presets/:id` deletes the preset's dead letters in the same batch as the votes and the preset itself. **`GET /api/v1/moderation/failed-notifications` changes shape**: each row is now `{ id, type, preset_id, moderation_status?, error, attempts, created_at, resolved_at }` instead of the raw row with its `payload` string, which also stops rows written before this release from re-publishing their author id and preset text while they age out. No consumer outside this worker reads that endpoint (the moderation-worker and discord-worker never call it). No migration: the `payload` column stays, it simply holds less.

- **Bot requests must now carry the v2 signature, and a nonce is single-use** (docs/audits/2026-08-29-security, FINDING-015, presets-api half). `authMiddleware` verified `X-Request-Signature-V2` when the header was present but fell back to v1 (`timestamp:userId:userName`, 5-minute window) when it was **absent** — and v1 binds nothing about the request: not the method, not the path, not the body. Anyone holding a single captured v1 tuple could therefore strip the v2 header and replay it against any route, with any method and any body, as that user for five minutes. The v2 nonce (`X-Request-Nonce`) was bound into the signature but never checked for reuse anywhere, so a captured v2 request could likewise be replayed verbatim for its 60 s window.

  v1 acceptance is gone: a bot request without a valid `X-Request-Signature-V2` is unauthenticated whatever `X-Request-Signature` carries, and gets the same 401 (unchanged envelope) an invalid signature has always produced. **No client change is required** — both bots have sent v2 alongside v1 since 2026-08-21 and were deployed on it on 2026-08-28; discord-worker and moderation-worker keep sending the now-ignored v1 header until a later sprint removes it along with the `verifyBotSignature` export in `@xivdyetools/auth`. Once a signature verifies, the nonce must be non-empty, at most 64 characters and `[A-Za-z0-9._-]` (the bots send a UUID) or the request is refused, and it is then recorded in the already-bound `TOKEN_BLACKLIST` KV namespace under a `botnonce:` prefix with a 120 s TTL (2× the signature window; KV's minimum is 60 s) — a second sighting of the same nonce is refused. No new namespace and no `wrangler.toml` change; nothing is written before a signature verifies, so an unauthenticated caller cannot fill the namespace. The replay cache is **best-effort**: KV is eventually consistent, so cross-colo detection is not guaranteed and the 60 s signature window remains the primary bound; a KV error — or the binding being absent, as in dev and tests — skips the *replay* check (logged, without the nonce) and never the signature check or the nonce format check. The JWT (web) path is untouched.

- **The request logger no longer collects the User-Agent header** (docs/audits/2026-08-29-security, FINDING-010). `loggerMiddleware({ ..., logUserAgent: true })` put every caller's User-Agent into the "Request started" log context on every request; worker-kit's own default is `false`, and nothing here ever consumed the value. The web privacy guide promises the server "discards everything about the request" — this field was the one exception. `logUserAgent` is no longer set, so the worker-kit default applies. The same finding's other half lived in `@xivdyetools/worker-kit` itself: the public rate limiter's fail-open log line carried the raw client IP. `@xivdyetools/worker-kit` 1.2.0 replaces it with a non-identifying bucket-class `keyScope` (`public:ip` here); this worker picks it up via `workspace:*` with no code change of its own.

- **`console.*` is gone from this worker's source — every log line now goes through the structured request logger, and none of them carry a name** (docs/audits/2026-08-29-security, FINDING-011). Roughly 30 `console.error` / `console.warn` / `console.info` / `console.log` call sites bypassed the redacting worker-kit logger entirely; two of them (`handlers/presets.ts`'s Discord-notification-failure paths) logged the submitter's own preset name (`name="${preset.name}"`) alongside the id. Every conversion follows the pattern already used in `services/preset-service.ts` — an optional `logger?` parameter (`(logger ?? console).method(...)`) in services that are also called without a request in scope, `c.get('logger')` / `getLogger(c)` directly in middleware and route handlers — so behavior outside the transport is unchanged, and existing tests (none of which wire up the real request logger) keep exercising the same `console` fallback they always did. Both notification-failure log lines now carry `presetId` only. `middleware/auth.ts`'s failed-signature warning renamed its `hasSignature` context field to `hasSignatureV2` — its meaning changed with the v2-only cutover above. `services/moderation-service.ts`'s Perspective-error log now carries the HTTP status only, never the upstream response body; a `throw` that existed purely so its `catch` block could log the same failure is now a direct log-and-return. `eslint.config.js` gained a scoped `no-console: 'error'` for `apps/presets-api/src/**` so a future edit can't reintroduce a bare call.

- **Production startup validation now requires the bindings the 2026-08-21 fixes depend on, and fails every request — not just logs — when one is missing** (docs/audits/2026-08-29-security, FINDING-013). `validateEnv()` never checked for `JWT_SECRET`, `JWT_ISSUER`, `TOKEN_BLACKLIST` or `RL_PUBLIC` in production, so a config edit or a dashboard change that silently dropped one of them degraded to "no JWT verification" / "no issuer pinning" / "no revocation check" / no nonce-replay cache (the FINDING-015 cache above rides the `TOKEN_BLACKLIST` binding) / a fallback in-memory rate limiter — with no error and no log anywhere. Production now additionally requires `JWT_SECRET` (non-empty, at least 32 characters), `JWT_ISSUER` (must start with `https://`), and the `TOKEN_BLACKLIST` and `RL_PUBLIC` bindings to be present, using the same `Missing required env var in production: X` shape `BOT_SIGNING_SECRET` already used. The env-validation middleware already failed every request in production when validation failed — not just the first one in the isolate (the BUG-017 pattern) — confirmed, not changed, by this release.

- **A wrangler-config invariant test now pins the shapes a deploy-time config drift could quietly break** (docs/audits/2026-08-29-security, FINDING-023). `tests/wrangler-config.test.ts` (new, modeled on `apps/api-worker/tests/wrangler-config.test.ts`) asserts the top-level worker stays a routeless `-dev` deploy (`workers_dev = false`, no top-level `routes`), production is `xivdyetools-presets-api` on `api.xivdyetools.app` with `JWT_ISSUER` / `ENVIRONMENT` pinned, the dev and production `TOKEN_BLACKLIST` KV ids actually match oauth's (not just each other), production binds an `RL_PUBLIC` ratelimit, and no `[env.preview]` block exists. None of this was guarded before; a binding silently pointed at the wrong namespace is exactly the drift FINDING-013's fail-closed `validateEnv` cannot catch on its own, since a wrong-but-present binding still passes a truthy check.

### Deploy notes

- **Apply migration `0012_submission_events_text_edit.sql` before deploying; at worst right after — the text-edit cap is inert until it is applied, and nothing else depends on it.** (`wrangler d1 execute xivdyetools-presets --remote --file=migrations/0012_submission_events_text_edit.sql`.) `submission_events.kind` carries a CHECK constraint listing only the three kinds migration `0011` knew about, and SQLite cannot alter a CHECK constraint — the migration rebuilds the table (copy, drop, rename, re-create the index) with `text_edit` added, and adds `idx_submission_events_created` for FINDING-017's age-based prune, which matches on `created_at` alone. The pre-`0012` CHECK still admits every kind 2.1.0 writes, so the deploy order is genuinely free: applied late, the `text_edit` INSERT is best-effort and its CHECK violation is caught and logged, so edits still succeed — but no row lands, the count stays 0 and the FINDING-005 cap does not engage until the migration runs. Verify the rebuild copied every row with `wrangler d1 execute xivdyetools-presets --remote --command "SELECT COUNT(*) FROM submission_events"` before and after. No `BEGIN TRANSACTION` / `COMMIT` (D1 rejects them; a `wrangler d1 execute --file` batch is atomic on its own), and `npm run db:migrate` will not apply it.
...
- **Three new curated categories**: `appearance` (👤 "Palettes built around a character's own colors"), `zones` (🏔️ "Palettes drawn from the places of Eorzea"), `raids-trials` (🗡️ "Palettes from raid and trial encounters") — seeded by migration `0010` and `schema.sql`.
...
- `validateEditRequest()` is now async and DB-backed like `validateSubmission()`, taking the preset's current `category_id` so a secondary that repeats the unchanged primary is caught; the "No updates provided" guard recognizes `example_link`, `category_id` and `secondary_categories`.
