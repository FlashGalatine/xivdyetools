# XIV Dye Tools Discord Bot - Terms of Service

**Last Updated**: December 7, 2025

## 1. Introduction

Welcome to XIV Dye Tools Discord Bot ("the Bot", "our Bot"). These Terms of Service ("Terms") govern your use of the Bot and its services. By using the Bot, you agree to be bound by these Terms.

The Bot provides color matching, harmony generation, and dye exploration tools for Final Fantasy XIV players.

## 2. Acceptance of Terms

By adding the Bot to your Discord server or using any of its commands, you acknowledge that you have read, understood, and agree to be bound by these Terms. If you do not agree to these Terms, do not use the Bot.

## 3. Description of Service

XIV Dye Tools Discord Bot provides:

- **Color Matching**: Find FFXIV dyes closest to any hex color or extracted from images
- **Color Harmonies**: Generate complementary, triadic, analogous, and other color schemes
- **Accessibility Tools**: Colorblindness simulation for dye colors
- **Community Preset Favorites**: Mark and list community presets you like (`/preset favorite`)
- **Community Presets**: Browse, submit, and vote on user-created color palettes
- **Market Pricing**: Real-time market board prices via Universalis API

The Bot is provided free of charge as a fan-made community tool.

## 4. User Conduct

When using the Bot, you agree to:

- Use the Bot only for its intended purpose
- Not attempt to abuse, exploit, or circumvent rate limits
- Not use the Bot for spam, harassment, or any malicious purpose
- Not attempt to reverse engineer, hack, or disrupt the Bot's functionality
- Not submit inappropriate, offensive, or copyrighted content to community features

### Rate Limits

The Bot implements rate limiting to ensure fair usage for all users. Attempting to circumvent these limits may result in temporary or permanent restrictions.

## 5. Content Moderation

### Community Presets

User-submitted presets are subject to moderation:

- Submissions pass an automated content check. Those that clear it are published immediately and logged for audit; anything flagged, or that the check cannot resolve, is held for moderator review
- Content that is offensive, inappropriate, or violates intellectual property rights will be rejected
- Moderators reserve the right to remove or modify any submitted content
- Repeated violations may result in loss of submission privileges

### Appeal Process

If you believe your content was incorrectly moderated:

1. Join our Discord server: https://discord.gg/rzxDHNr6Wv
2. Contact a moderator or ping "Flash Galatine" in #dyetools-issues-and-suggestions
3. Provide your Discord user ID and the name of the rejected preset
4. Appeals are reviewed within 7 days

## 6. Intellectual Property

### Square Enix Content

FINAL FANTASY XIV © SQUARE ENIX CO., LTD. All Rights Reserved.

This Bot is a fan-made tool and is **not affiliated with, endorsed by, or sponsored by Square Enix Co., Ltd.** FINAL FANTASY is a registered trademark of Square Enix Holdings Co., Ltd.

Game data, including dye names, colors, and acquisition methods, are property of Square Enix Co., Ltd. and are used under fair use for educational and informational purposes only.

### Third-Party Services

Market board data is provided by [Universalis](https://universalis.app/), an independent third-party service not affiliated with Square Enix.

### Your Content

By submitting presets or other content to the Bot, you grant us a non-exclusive, royalty-free license to use, display, and distribute that content within the XIV Dye Tools ecosystem.

## 7. Disclaimer of Warranties

THE BOT IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED.

We do not guarantee:

- Continuous, uninterrupted availability
- Accuracy of color matching or market data
- Compatibility with all Discord features or clients
- Any specific level of performance or response time

## 8. Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW, WE SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES ARISING FROM YOUR USE OF THE BOT.

This includes but is not limited to:

- Loss of data or content
- Service interruptions
- Inaccurate information
- Actions taken based on Bot recommendations

## 9. Modifications to Service

We reserve the right to:

- Modify, suspend, or discontinue the Bot at any time without notice
- Add, change, or remove features
- Update these Terms as needed

Continued use of the Bot after changes constitutes acceptance of the modified Terms.

## 10. Termination

We may, at our sole discretion, suspend or terminate your access to the Bot for:

- Violation of these Terms
- Abuse or misuse of the service
- Behavior that negatively impacts other users
- Any other reason we deem necessary

## 11. Governing Law

These Terms are governed by applicable laws. Any disputes will be resolved through good-faith negotiation.

## 12. Contact

For questions about these Terms or the Bot:

- **Email**: FlashGalatineFGC@gmail.com (Subject: "XIV Dye Tools Terms")
- **Discord**: https://discord.gg/rzxDHNr6Wv
- **Support Channel**: #dyetools-issues-and-suggestions

---

**By using XIV Dye Tools Discord Bot, you acknowledge that you have read and understood these Terms of Service.**
