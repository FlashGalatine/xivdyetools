...
- **`/swatch` rejected every Lalafell `.chara` attachment** with "`.chara` field Race: unrecognised
  ...
  parser treated the `Race` key as authoritative, so it threw before reading a color. Broken
  since `/swatch` began taking file attachments, not a regression. Core now derives the race
  from the tribe, which does not drift.
- **A Hrothgar attachment with no `Race` key had its fur-pattern number matched as a lip color**,
...
- 5.6.0's I18N-002 note said "the emoji prefix is kept" for `/manual topic`; only the English names
  keep it — the localized names are text-only. Wording corrected, no behavior change.
...
- **I18N-006**: `/extractor image` printed "1 colors" / "1 Farben" when K-means collapsed to a
...
- Also through bot-logic 4.4.0: the "NEAREST DYE" label on the harmony, swatch and extractor cards
  ...
  Chinese had a modifier with no noun; German's swatch column said `FARBE`, color, for a dye);
...
- `localize.test.ts` asserts coverage, not a count: every command, subcommand, group and option is
  ...
  English. Plus the six `/manual` topics × five locales, the `/about` field, and the one-color path.
...
2026-09-16 deep-dive Sprint 4 (`docs/audits/2026-09-16-deep-dive/`). Picks up `@xivdyetools/bot-logic` 4.3.0 (`/swatch` eye rows labeled `EYES·L` / `EYES·R` / `EYES·LR` so off-grid heterochromia rows are told apart, BUG-006). The `/preset` schema change means `register-commands` must run with the deploy.
...
- Harmony and color-vision names in the command picker now come from `@xivdyetools/core`
...
- **`/gradient` step and `/harmony` base color chips render** (BUG-032, BUG-033). Both passed an
...
- **Favorites autocomplete survives a failed name back-fill** (BUG-028). The one-time migration was
...
- This app is now gated on the monorepo's `knip` dead-code check (`pnpm run lint:dead`, folded
  ...
  type, same runtime behavior, now visible to the gate.
...
- **Rate-limit counters left Upstash Redis for Cloudflare's own rate-limiting service** (`docs/audits/2026-08-29-security` FINDING-007). The limiter preferred Upstash whenever `UPSTASH_REDIS_REST_URL`/`_TOKEN` were set — which production was — so a per-user, per-command counter keyed by Discord user id was written to a third party the bot's privacy policy never named as a processor, while the policy said the counters lived in Cloudflare KV. The worker now counts against native `[[ratelimits]]` bindings (`RL_5`, `RL_10`, `RL_15`, `RL_20`, `RL_30`, `RL_70` — one tier per distinct per-minute limit in worker-kit's `DISCORD_COMMAND_LIMITS`, six per environment), which count in-process per colo with no storage write and no third party; KV remains only as the fallback when no tier is bound (tests / local dev), and the Upstash code path, config fields and `Env` entries are gone. A **partial** set of tiers is now warned about too — once per isolate, on the request logger, naming only the missing binding names (`Rate limiter: some RL_* bindings are missing — affected commands use the next larger tier`): that shape never reaches the KV fallback, because worker-kit keeps the Cloudflare backend and routes the orphaned commands to the next *larger* tier, so a dashboard edit that dropped `RL_5` would quietly hand `/extractor image` 10/min instead of 5 with nothing in the logs. The two secrets are retired — delete them from the production worker after the deploy (`docs/operations/SECRET_ROTATION.md`, `POST_MERGE_CHECKLIST.md`).
- **`/about`, `/manual` and `/changelog` are no longer exempt from the limiter** (FINDING-020). The exemption skipped only the rate-limit check, not the three shared hot-key KV counter writes every finished command performs — KV allows one write per second per key, so the exempt commands were the cheapest way to hammer the worker's hottest keys and skew the public `/stats`. All three now take the 30/min tier worker-kit already defined for `/about` and `/manual`; `/changelog` gets it from a local override in `services/rate-limiter.ts` until the preset itself gains the entry. A limited call follows the normal path: an ephemeral "too quickly" reply and an Analytics-Engine-only `rate_limited` row.
- **Production requires all six rate-limit bindings — a missing one is an error, not a shrug** (`docs/audits/2026-08-29-security` FINDING-013). With the counters now living on the `[[ratelimits]]` bindings, a production worker that lost one of them kept serving: the limiter stayed on the Cloudflare backend and simply handed the orphaned commands a larger allowance, and losing all six fell back to a KV limiter that cannot throttle a fast client at all. The once-per-isolate warning above is the only in-process signal, and Workers Logs are off on this script — so the check has to be config-level. `wrangler.toml` now declares `ENVIRONMENT` in **both** blocks (`"development"` top-level / beta, `"production"` under `[env.production]` — `vars` are not inheritable, so it is repeated rather than inherited), and `validateEnv` requires `RL_5`, `RL_10`, `RL_15`, `RL_20`, `RL_30` and `RL_70` when it reads `production`, pushing `Missing required env var in production: RL_N` for each one that is unbound. **A production worker refuses every request while one of those errors stands** — `500 {"error":"Service misconfigured"}`, `/health` included, indistinguishable from a missing `DISCORD_TOKEN`, and re-checked on every request rather than once per isolate (BUG-017). A logged line would not have been a signal: Workers Logs are off on this script, so the bot would have gone on quietly handing out the wrong allowances; an interaction that answers an error is noticed in minutes. The beta worker is untouched — those errors can only be raised under `ENVIRONMENT === "production"`, so `development` keeps the log-only path and its KV fallback. Nothing else in the worker reads `ENVIRONMENT` — `loggerMiddleware` runs with `readEnvironmentFromEnv: false` and keeps its `production` log level in both environments — so the beta bot is unaffected apart from the new var itself. `tests/wrangler-config.test.ts` pins both values, and now checks the TOML's six `simple.limit`s against the exported `BINDING_TIERS` in `services/rate-limiter.ts` instead of a second copy of the same literals.
- **The first-run notice flag was a permanent per-user KV record — it now expires after 180 days** (`docs/audits/2026-08-29-security` FINDING-008). `maybeSendFirstRunNotice()` wrote `firstrun:v5:<userId>` with no `expirationTtl`, so every user who had ever run a command left a KV row forever, and no test asserted on the write. The put now carries `expirationTtl: FIRST_RUN_FLAG_TTL_SECONDS` (180 days = 15,552,000 seconds); a user who returns after the flag has expired with no stored preferences may see the notice once more, which is acceptable. `PRIVACY_POLICY.md` was also out of date since the 5.0 command cull: it listed the removed `Favorite Dyes` and `Collections` records and pointed users at `/favorites`, `/collection` and `/match_image`, none of which exist any more, while never mentioning the two records that actually accumulate per-user data (`Preferences`, preset favorites) or the first-run flag itself, and said rate-limit counters live "70 seconds in KV" (now neither true — see FINDING-007 above). It now lists preset favorites, the first-run flag and the real preference fields, states that per-user counters live in Cloudflare's rate-limiting service with KV only as the fallback, and cites `/preferences show`, `/preset favorite list`, `/preferences reset` and `/preset favorite remove` for access and deletion.
- **The default world is validated when it is set and again when it is read** (`docs/audits/2026-08-29-security` FINDING-019). `/preferences set world:` accepted any non-empty string — the option carried no `max_length`, so Discord allowed up to 6000 characters — and stored it verbatim in `prefs:v1:<userId>`; `/budget find|quick` then returned that stored value unchecked (only an explicit `world:` override was validated, FINDING-033) straight into the Universalis proxy query and the shared price-cache key. All four registered `world` options now publish `max_length: 32`, the synchronous guard behind them accepts a trimmed 1–32 characters with no control characters (a length check, never an ASCII one — `红玉海` and `카벙클` are real worlds), `/preferences set world:` runs the same Universalis lookup `/budget set_world` always did and stores the **canonical** name (`balmung` → `Balmung`), and `resolveWorld()` validates the stored preference too — a world that no longer resolves is answered with the existing "Could not find world or datacenter" reply, naming the stored value, instead of being priced. The lookup is the hour-cached one, so this costs at most one world-list fetch per isolate per hour. **The command schemas must be re-registered for the cap to reach Discord** — `deploy-discord-worker.yml` runs `register-commands` itself after a production deploy, so merging is enough.
- **Log lines carry ids and lengths, not option values or preset names** (FINDING-011, discord-worker part). Three structured log lines wrote user-supplied text into the log sink: the `/budget` ledger line logged the world name, `setPreference()`'s failure path logged the preference *value* (home world, clan, language), and the preset webhook logged the submitted preset's name — free text about a submission that may never be published. They now log `{ targetDyeId, hasWorld }`, `{ key, valueType, valueLength }` and `{ presetId, source }`. A sweep of every `logger.*` call in the app found no other line logging a name, option value or free text; Discord's own error-response body on a failed follow-up edit (`utils/discord-api.ts`, `index.ts`) names the rejected field and its constraint, never the submitted value, and stays.
- **The release-announcement webhook is pinned to this repository, takes `push` events only, and announces each version once** (`docs/audits/2026-08-29-security` FINDING-021). `POST /webhooks/github` verified GitHub's HMAC and then trusted the payload for everything after it: the changelog was fetched from `raw.githubusercontent.com/${payload.repository.full_name}/…` and `payload.repository.html_url` became the masked "full release notes" link and the embed footer, so anyone holding — or sharing — `GITHUB_WEBHOOK_SECRET` could post any repository's notes behind any link in an announcement that looked like ours; no `X-GitHub-Event` was read, so any signed event whose JSON happened to carry `ref`/`commits`/`repository` was processed; and nothing remembered what had already gone out, so a *Redeliver* — which the release flow needs, because the hook fires before the deploy finishes — re-posted the same release (three times on 2026-08-29). The repository and its URL are now the constants `GITHUB_ANNOUNCE_REPO` / `GITHUB_ANNOUNCE_REPO_URL`; a payload naming any other repository is refused **403** `Repository not allowed` (logged as `{ repoAllowed: false }` — never the offending name), a `ping` answers `pong` and any other event `Ignored event` without the body being parsed (2xx, so the hook stays healthy in GitHub's delivery log), and a version that has already been announced is skipped via KV `announced:v:<version>` (90-day TTL, written only after a successful send, so a failed announcement can still be retried). The memo write is itself best-effort: the release has already gone out by the time it runs, so a KV failure is warned and still answered 200 rather than 500 — a failed delivery would invite exactly the *Redeliver* that double-posts. The HMAC check and the 1 MiB body cap are unchanged.
- **The bot stopped sending the legacy v1 request signature** (`docs/audits/2026-08-29-security` FINDING-015). Every signed call to presets-api carried both the old `X-Request-Signature` (a bare `timestamp:userId:userName` HMAC that bound nothing about the request itself — method, path and body could change underneath it without invalidating it) and the newer `X-Request-Signature-V2` (FINDING-014, 2026-08-21 audit); v1 was meant to be temporary rollover cover, but nothing ever removed it. presets-api stopped accepting v1 in 2.2.0 (Sprint 1 of this audit), so the header was dead weight that kept a weaker, unbound signature scheme nominally in circulation for no purpose. `generateRequestSignature()` and the `X-Request-Signature` header are gone from `services/preset-api.ts`; a signed request now carries only the timestamp, nonce and v2 signature it already computed. moderation-worker's own copy of this client still sends v1 (Sprint 4); `@xivdyetools/auth`'s v1 export (`verifyBotSignature`) was removed in Sprint 11.

### Added — 2026-08-29 analytics (Tier A)

- **Command analytics describe the real work, not the deferred ack** (spec `docs/superpowers/specs/2026-08-29-bot-analytics-tier-a-design.md`, queries `docs/operations/ANALYTICS_QUERIES.md`). The dispatcher now owns a `CommandTrace` (`services/command-trace.ts`): every handler gets a traced `ExecutionContext`, and the datapoint is written once the handler's background work settles — bounded by a 20 s drain deadline after which the row is written as `unknown` rather than lost. New Analytics Engine columns, all additive: `blob5` outcome class (`ok`, `rejected`, `rate_limited`, `upstream_universalis`, `upstream_presets`, `image_input`, `render`, `unknown`; never an error message), `blob6` subcommand (groups as `<group>_<sub>`) or copy-button kind, `blob7` Discord client-locale bucket, `blob8` `command` \| `button`, `double2` real latency. Copy-button clicks are AE-only `kind=button` rows. No command option value, hex, search text, world, image name or guild/channel id is recorded — `PRIVACY_POLICY.md` §2 lists exactly what is.
- **Outcome rules — two axes**: `blob4` (and the KV counters behind `/stats`) says whether the user got an answer; `blob5` says what of ours broke. A 4xx other than 429 from presets-api / Universalis is `rejected` (the service's own reply, relayed — answered, but its own class so a systematic 4xx from our payload is a visible spike); a lost card is `render`, and `/dye`'s text fallbacks are `render` *and* answered (`served`); `image_input` covers image-worker's rejections (one marker table, `services/image-input-errors.ts`, which also picks the `/extractor image` user message — its "timed out" marker now matches the worker's real wording) and an unreadable `/swatch` attachment. Rate-limited requests are Analytics-Engine-only: they no longer touch the KV counters behind `/stats`.
- The image-worker and presets-api service-binding calls carry a 10 s `AbortSignal.timeout`; `/stats` now `return await`s its subcommands so a KV outage is answered with the localized "could not fetch" embed instead of the dispatcher's generic failure.

### Fixed — 2026-08-29

- **Every card's `font-weight` was a no-op — titles shipped in Light.** The Worker bundled the *variable* Space Grotesk and Onest files, and resvg's font database cannot move a variable axis: each file exposed only its default instance, so the 600 on every heading and the 700 on dye names rendered at that one weight — and Space Grotesk's default instance is **Light 300**, so every card title has been thin since 5.0. The brand faces now ship as static instances (`SpaceGrotesk-{Regular,SemiBold,Bold}.ttf`, `Onest-{Regular,SemiBold,Bold}.ttf`, generated by the new `scripts/instance-latin-fonts.py` from the variable sources moved to `scripts/font-sources/`), which fontdb matches by `usWeightClass`. Bundle: +185 KiB raw (six 63–85 KiB statics replacing two 120–131 KiB variable files); gzip figure in the PR. New `font-faces.test.ts` renders 400 / 600 / 700 through the real resvg-wasm with exactly the faces `fonts.ts` imports and fails if any two match, or if a variable font reappears. (`apps/og-worker` has the same setup and gets the same fix separately.)
- **`/dye info` card was unreadable on light dyes.** The header band is painted in the dye's own color and its text was always white, so on Pure White, Snow White, Honey Yellow and some 55 other light dyes the name, stain number, category and command pill all but vanished. The band's ink is now picked per dye by measured contrast (`@xivdyetools/svg` `bandInk` / `pillInkOnDye`); dark dyes render exactly as before.
...
- **FINDING-019 — user text in bot embeds**: every user-sourced string that reaches a Discord embed goes through `@xivdyetools/bot-logic`'s `sanitizeEmbedText` (control / zero-width / bidi characters stripped, `@everyone` / `@here` / `<@…>` defused, markdown incl. masked links escaped, length capped); `utils/sanitize.ts`'s `sanitizePresetName` / `sanitizePresetDescription` now delegate to it. Covered: `/preset show|list|submit|edit|favorite` (stored name / description / tags / author, the duplicate-preset echoes, typed dye names), the auto-approved submission-log embed and the webhook's author / tags fields (both had skipped the sanitizer the moderation path uses), the `/swatch` parse-error embed (the parser echoes `.chara` field *values*), the `/dye search` query title and the `/budget` echoes. The preset swatch PNG still receives the raw text — the SVG layer XML-escapes and would otherwise print backslashes. Every outbound payload in `utils/discord-api.ts` (follow-up / edit / channel message / multipart `payload_json`) now carries `allowed_mentions: { parse: [] }` unless the caller passes an explicit `allowedMentions`.
- **FINDING-020 — preset ids in URL paths**: `services/preset-api.ts` percent-encodes every preset-id path segment (`getPreset`, `editPreset`, `voteForPreset`, `removeVote`, `hasVoted`, `setPreviewImageStatus`) so `..` / `/` / `?` / `#` cannot steer a bot-authenticated request onto another route, and handlers validate at the boundary (`isValidPresetId`, `types/preset.ts`): a `/preset show|vote|edit|favorite` value that is not a UUID is a typed *name* and is resolved through the search query (never a path segment), and the preview-image moderation buttons refuse a non-UUID `custom_id` before authorization or any API call.
...
- **Localized dye-name input**: autocomplete and typed dye names match English *or* the user's locale (`searchDyesByName` / `findDyeByName`), choices are labeled with the localized name; every handler passes `t.getLocale()` into `resolveColorInput` (F-02).
...
- **`utils/brand.ts`**: `BRAND_ACCENT` (#EA4133) + a `STATE` vocabulary (success/warning/error/neutral/confirm) — one home for embed colors; blurple was declared in five files and used as a literal in three more.
...
- **`/extractor` options now do what the schema promises** — `matching` drives the ranking (`color`), the per-slot dye pick and every displayed distance (`image`), and the 14J·2 / 14K cards render that method's tier bars, column tag and "nearest by …" key line (`@xivdyetools/svg` method-aware rows); resolution is option → stored `/preferences` method → ΔE2000, like `/harmony`. `prevent_duplicates` on `image` is honored (default on; `false` keeps each slot's true nearest dye). Removed from the schema: `vibrancy_boost` (never implemented anywhere) and `prevent_duplicates` on `color` (a nearest-N of distinct dyes cannot repeat) — `register-commands` runs in CI on merge
