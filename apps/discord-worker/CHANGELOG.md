...
- **`/changelog`** (new, ephemeral): renders the root `CHANGELOG-laymans.md` via `parseAll()` — newest release expanded, five collapsed one-liners, `version:` expands any recorded release. Fetched from the repo raw URL with a 10 s timeout and a 10-minute KV cache; skipped by the rate limiter alongside `about` / `manual` / `stats`.
...
- **Component context store** (`services/component-context.ts`): rewritten from the per-colo Cache API (BUG-075) to KV (`ctx:v2:{hash}`, `expirationTtl` 15 min); interaction tokens are no longer stored; `verifyContextUser` guards hash collisions.
...
- **Release announcement** (`services/announcements.ts`): a long release now cuts on a line boundary and says it is a summary pointing at `/changelog`, instead of truncating mid-bullet behind a bare `...`; `changelog-parser.ts` parses the whole history (`parseAll()`, `parseLatestVersion = parseAll()[0]`) against the strict root `CHANGELOG-laymans.md` contract.
