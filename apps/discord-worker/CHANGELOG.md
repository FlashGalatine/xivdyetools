...
- **4 dead factories in `src/test-utils.ts`** (DEAD-005): `createMockExecutionContext`, `createMockDye`, `createMockDyes`, `createMockPreset` — zero non-test consumers; `createMockKV` / `createMockD1` / `createMockAnalytics` / `createMockEnv` stay (Task 5 territory).
