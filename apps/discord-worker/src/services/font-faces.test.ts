 ...
 * file exposes exactly its default instance, so every `font-weight` the card
 * system asks for (600 on headings, 700 on dye names) silently rendered at
 * that single weight — and Space Grotesk's default instance is Light (300),
 * so every card title shipped thin. This renders the same word at 400 / 600 /
 * 700 through the real resvg-wasm, with precisely the faces `fonts.ts`
 * imports, and demands the three weights come out different.
 */
