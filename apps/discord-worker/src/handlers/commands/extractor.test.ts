...
function makeInteraction(colorOptions: Array<{ name: string; value: unknown }>): DiscordInteraction {
  return {
    id: 'i-1',
    token: 'tok',
    type: 2,
...
describe('/extractor color — matching method', () => {
  ...
  it('defaults to ΔE2000 and passes it to the card', async () => {
    ...
    await handleExtractorCommand(makeInteraction([{ name: 'color', value: '#4A6B8C' }, { name: 'count', value: 3 }]), env, ctx);
    await flush();
    ...
    for (const r of opts.rows) {
      expect(r.deltaE).toBeCloseTo(ColorService.getDistanceForMethod('#4A6B8C', r.hex, 'ciede2000'), 6);
    }
    expect(opts.rows.map((r) => r.deltaE)).toEqual([...opts.rows.map((r) => r.deltaE)].sort((a, b) => a - b));
  });

  it('honours an explicit matching option (ranking + card tag)', async () => {
    const { ctx, flush } = makeCtx();
    await handleExtractorCommand(
      makeInteraction([{ name: 'color', value: '#4A6B8C' }, { name: 'count', value: 3 }, { name: 'matching', value: 'redmean' }]),
      env,
      ctx
    );
    await flush();
    ...
    for (const r of opts.rows) {
      expect(r.deltaE).toBeCloseTo(ColorService.getDistanceForMethod('#4A6B8C', r.hex, 'redmean'), 6);
    }
  });
