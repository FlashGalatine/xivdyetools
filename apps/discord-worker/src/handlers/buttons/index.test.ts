...
vi.mock('./copy.js', () => ({
  handleCopyHex: vi.fn(
    () => new Response(JSON.stringify({ type: 4, data: { content: 'hex copied' } })),
  ),
  handleCopyRgb: vi.fn(
    () => new Response(JSON.stringify({ type: 4, data: { content: 'rgb copied' } })),
  ),
  handleCopyHsv: vi.fn(
    () => new Response(JSON.stringify({ type: 4, data: { content: 'hsv copied' } })),
  ),
...
describe('buttons/index.ts', () => {
  ...
  describe('handleButtonInteraction', () => {
    ...
    it('should route previewimg_approve_ buttons to handlePreviewImageButton', async () => {
      ...
      expect(handlePreviewImageButton).toHaveBeenCalledWith(
        interaction,
        mockEnv,
        mockCtx,
        undefined,
    ...
    it('should log info when logger is provided', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Handling button', {
        customId: 'copy_hex_FF5733',
      });
    });
    ...
    it('should log warning for unknown buttons when logger is provided', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'Unknown button custom_id: totally_unknown_button',
      );
    });
  });
});
