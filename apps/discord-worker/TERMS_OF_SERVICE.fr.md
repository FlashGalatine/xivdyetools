# XIV Dye Tools Discord Bot - Conditions d'utilisation

> Ceci est une traduction fournie à titre pratique. La version anglaise fait foi ; en cas de divergence, c'est elle qui prévaut. [Anglais](TERMS_OF_SERVICE.md)

**Dernière mise à jour** : 2026-09-16

> L'application web sur xivdyetools.app a ses propres conditions : [`apps/web-app/TERMS_OF_SERVICE.md`](../web-app/TERMS_OF_SERVICE.md). Le traitement des données est couvert par la [Politique de confidentialité](PRIVACY_POLICY.md).

## 1. Introduction

Bienvenue sur XIV Dye Tools Discord Bot (« le Bot », « notre Bot »). Ces conditions d'utilisation (« Conditions ») régissent votre utilisation du Bot et de ses services. En utilisant le Bot, vous acceptez d'être lié par ces Conditions.

Le Bot fournit des outils de correspondance de couleurs, de génération d'harmonies et d'exploration de teintures pour les joueurs de Final Fantasy XIV.

## 2. Acceptation des conditions

En ajoutant le Bot à votre serveur Discord ou en utilisant l'une de ses commandes, vous reconnaissez avoir lu, compris et accepté d'être lié par ces Conditions. Si vous n'acceptez pas ces Conditions, n'utilisez pas le Bot.

## 3. Description du service

XIV Dye Tools Discord Bot fournit :

- **Correspondance de couleurs** : trouver les teintures FFXIV les plus proches de n'importe quelle couleur hexadécimale ou extraite d'images
- **Harmonies de couleurs** : générer des schémas de couleurs complémentaires, triadiques, analogues et autres
- **Outils d'accessibilité** : simulation du daltonisme pour les couleurs de teinture
- **Favoris de palettes prédéfinies communautaires** : marquer et lister les palettes prédéfinies communautaires que vous aimez (`/preset favorite`)
- **Palettes prédéfinies communautaires** : parcourir, soumettre et voter sur des palettes de couleurs créées par les utilisateurs
- **Prix du marché** : prix du tableau des ventes en temps réel via l'API Universalis

Le Bot est fourni gratuitement en tant qu'outil communautaire créé par des fans.

## 4. Conduite de l'utilisateur

En utilisant le Bot, vous acceptez de :

- Utiliser le Bot uniquement pour son usage prévu
- Ne pas tenter d'abuser, d'exploiter ou de contourner les limites de débit
- Ne pas utiliser le Bot pour du spam, du harcèlement, ou toute finalité malveillante
- Ne pas tenter de rétro-concevoir, pirater, ou perturber le fonctionnement du Bot
- Ne pas soumettre de contenu inapproprié, offensant, ou protégé par le droit d'auteur aux fonctionnalités communautaires

### Limites de débit

Le Bot met en œuvre une limitation de débit pour garantir un usage équitable pour tous les utilisateurs. Toute tentative de contourner ces limites peut entraîner des restrictions temporaires ou permanentes.

## 5. Modération de contenu

### Palettes prédéfinies communautaires

Les palettes prédéfinies soumises par les utilisateurs sont soumises à modération :

- Les soumissions passent un contrôle de contenu automatisé. Celles qui le franchissent sont publiées immédiatement et journalisées pour audit ; tout ce qui est signalé, ou que le contrôle ne peut résoudre, est mis en attente pour un examen par un modérateur
- Le contenu offensant, inapproprié, ou qui viole des droits de propriété intellectuelle sera rejeté
- Les modérateurs se réservent le droit de supprimer ou de modifier tout contenu soumis
- Des violations répétées peuvent entraîner la perte des privilèges de soumission

### Processus d'appel

Si vous pensez que votre contenu a été modéré à tort :

1. Rejoignez notre serveur Discord : https://discord.gg/rzxDHNr6Wv
2. Contactez un modérateur ou mentionnez « Flash Galatine » dans #dyetools-issues-and-suggestions
3. Indiquez votre identifiant utilisateur Discord et le nom de la palette prédéfinie rejetée
4. Les appels sont examinés sous 7 jours

## 6. Propriété intellectuelle

### Contenu Square Enix

FINAL FANTASY XIV © SQUARE ENIX CO., LTD. All Rights Reserved.

Ce Bot est un outil créé par des fans et n'est **pas affilié à, approuvé par, ou parrainé par Square Enix Co., Ltd.** FINAL FANTASY est une marque déposée de Square Enix Holdings Co., Ltd.

Les données du jeu, y compris les noms de teintures, les couleurs et les méthodes d'acquisition, sont la propriété de Square Enix Co., Ltd. et sont utilisées dans le cadre d'un usage loyal (fair use) à des fins éducatives et informatives uniquement.

### Services tiers

Les données du tableau des ventes sont fournies par [Universalis](https://universalis.app/), un service tiers indépendant non affilié à Square Enix.

### Votre contenu

En soumettant des palettes prédéfinies ou tout autre contenu au Bot, vous nous accordez une licence non exclusive et libre de redevances pour utiliser, afficher et distribuer ce contenu au sein de l'écosystème XIV Dye Tools.

## 7. Exclusion de garanties

LE BOT EST FOURNI « EN L'ÉTAT » ET « SELON DISPONIBILITÉ », SANS GARANTIE D'AUCUNE SORTE, EXPRESSE OU IMPLICITE.

Nous ne garantissons pas :

- Une disponibilité continue et ininterrompue
- L'exactitude de la correspondance de couleurs ou des données de marché
- La compatibilité avec toutes les fonctionnalités ou clients Discord
- Un niveau spécifique de performance ou de temps de réponse

## 8. Limitation de responsabilité

DANS TOUTE LA MESURE PERMISE PAR LA LOI, NOUS NE SERONS PAS RESPONSABLES DE TOUT DOMMAGE INDIRECT, ACCESSOIRE, SPÉCIAL, CONSÉCUTIF, OU PUNITIF DÉCOULANT DE VOTRE UTILISATION DU BOT.

Cela inclut, sans s'y limiter :

- La perte de données ou de contenu
- Les interruptions de service
- Les informations inexactes
- Les actions entreprises sur la base des recommandations du Bot

## 9. Modifications du service

Nous nous réservons le droit de :

- Modifier, suspendre, ou interrompre le Bot à tout moment sans préavis
- Ajouter, modifier, ou supprimer des fonctionnalités
- Mettre à jour ces Conditions selon les besoins

Continuer à utiliser le Bot après des modifications vaut acceptation des Conditions modifiées.

## 10. Résiliation

Nous pouvons, à notre seule discrétion, suspendre ou résilier votre accès au Bot pour :

- Violation de ces Conditions
- Abus ou usage impropre du service
- Comportement portant atteinte négativement à d'autres utilisateurs
- Toute autre raison que nous jugeons nécessaire

## 11. Droit applicable

Ces Conditions sont régies par les lois de l'État de Caroline du Nord, États-Unis, sans égard à ses règles de conflit de lois. Tout litige qui ne peut d'abord être résolu par une négociation de bonne foi sera porté devant les tribunaux d'État ou fédéraux situés en Caroline du Nord, et vous acceptez cette juridiction compétente.

Rien dans ces Conditions ne retire un droit de protection du consommateur que votre droit local vous accorde et auquel il ne vous permet pas de renoncer.

## 12. Contact

Pour toute question sur ces Conditions ou sur le Bot :

- **E-mail** : FlashGalatineFGC@gmail.com (Objet : « XIV Dye Tools Terms »)
- **Discord** : https://discord.gg/rzxDHNr6Wv
- **Canal d'assistance** : #dyetools-issues-and-suggestions

---

**En utilisant XIV Dye Tools Discord Bot, vous reconnaissez avoir lu et compris ces conditions d'utilisation.**
