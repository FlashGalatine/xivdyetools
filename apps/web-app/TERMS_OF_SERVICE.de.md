# XIV Dye Tools — Nutzungsbedingungen (Web-App)

> Dies ist eine zur Verständlichkeit bereitgestellte Übersetzung. Maßgeblich ist die englische Fassung; weichen beide voneinander ab, gilt die englische Fassung. [Englisch](TERMS_OF_SERVICE.md)

**Zuletzt aktualisiert:** 2026-09-16 · Gilt für **xivdyetools.app** und **beta.xivdyetools.app**. Wie
wir mit deinen Daten umgehen, ist ein eigenes Dokument: [`PRIVACY.md`](PRIVACY.md). Der
Discord-Bot hat eigene Bedingungen:
[`apps/discord-worker/TERMS_OF_SERVICE.md`](../discord-worker/TERMS_OF_SERVICE.md).

XIV Dye Tools ist ein kostenloses, von Fans gemachtes Farb-Werkzeugset für Final Fantasy XIV. Es
steht in keiner Verbindung zu Square Enix und wird von diesem weder unterstützt noch gesponsert.
Mit der Nutzung der Website stimmst du dem Folgenden zu. Wenn du nicht zustimmst, nutze sie bitte
nicht — dich davon zurückzuziehen kostet dich nichts.

## Was die Website tut

Neun Werkzeuge laufen in deinem Browser: Paletten-Extraktor, Harmonie-Explorer, Vergleich,
Verlauf-Ersteller, Farbstoffmixer, Barrierefreiheitsprüfung, Budget-Finder, Swatch-Matcher und der
Community-Presets-Browser. Daneben kann die Website Marktbrett-Preise anzeigen, eine
`.chara`-Datei lesen, die du hineinziehst, um die Ausrüstung einer Mirage zu benennen, und dich
Paletten und Sammlungen auf deinem eigenen Gerät speichern lassen.

Das meiste davon funktioniert ohne Konto und ohne irgendetwas irgendwohin zu senden. Die
Ausnahmen sind in [`PRIVACY.md`](PRIVACY.md) aufgeführt.

## Konten

Du brauchst nie ein Konto, um die Farbwerkzeuge zu nutzen. Ein Konto benötigst du nur, um
Community-Presets einzureichen, zu bearbeiten oder darüber abzustimmen.

- Die Anmeldung läuft über Discord oder XIVAuth. Wir sehen oder speichern niemals ein Passwort.
- Die Anmeldung legt einen Kontodatensatz mit deiner Anbieter-ID und deinem Benutzernamen an. Dein
  Benutzername wird als Autor bei Presets angezeigt, die du veröffentlichst.
- Deine Sitzung ist ein Token, das in deinem Browser gehalten wird. Das Löschen der Website-Daten
  deines Browsers meldet dich ab.
- Du bist verantwortlich für das, was unter deinem Konto geschieht. Wenn du glaubst, dass jemand
  anderes Zugriff darauf hat, entziehe der App den Zugriff bei deinem Anbieter und teile es uns mit.
- Wir können ein Konto sperren oder entfernen, das gegen diese Bedingungen verstößt.

## Community-Presets

### Deine Inhalte bleiben deine

Du behältst das Eigentum an jedem Preset, das du einreichst. Mit dem Einreichen eines Presets
gewährst du uns eine nicht-exklusive, gebührenfreie Lizenz, diesen Inhalt innerhalb des
XIV-Dye-Tools-Ökosystems zu nutzen, anzuzeigen und zu verbreiten — der Website, dem Discord-Bot,
der API und Link-Vorschauen. Diese Lizenz besteht, damit wir dein Preset anderen Spielern zeigen
können; sie ist keine Eigentumsübertragung, und wir beanspruchen kein Recht, es zu verkaufen oder
weiter zu lizenzieren.

Reiche nur Inhalte ein, zu deren Einreichung du berechtigt bist.

### Worum wir dich bitten

Reiche nichts ein, das anstößig, belästigend, sexuell explizit, hasserfüllt, täuschend, werbend
oder eine Verletzung der Rechte Dritter ist. Nutze den Preset-Namen oder die Beschreibung nicht als
Kanal für etwas, das nichts mit Farbstoffen zu tun hat.

### Moderation

Einreichungen durchlaufen automatisierte Inhaltsprüfungen, bevor sie erscheinen. Eine Einreichung,
die sie besteht, wird sofort veröffentlicht und zur Prüfung protokolliert. Eine, die markiert wird,
wird stattdessen für einen menschlichen Moderator zurückgehalten, ebenso eine, bei der eine Prüfung
läuft, aber zu keinem Ergebnis kommt — etwa bei einem Dienstausfall. Im Zweifel geht nichts live.

Moderatoren dürfen jede Einreichung entfernen oder bearbeiten, und wiederholte Verstöße können dich
die Möglichkeit zum Einreichen kosten. Wir können Inhalte auch aus Gründen entfernen, die nichts
mit dir zu tun haben — einer rechtlichen Anfrage, oder einer Änderung dessen, was die Website
hostet.

### Widerspruch und Entfernung

Wenn du glaubst, dass dein Inhalt zu Unrecht moderiert wurde, oder du möchtest, dass etwas, das du
eingereicht hast, entfernt wird:

1. Tritt dem Discord bei: <https://discord.gg/rzxDHNr6Wv>
2. Frag in **#dyetools-issues-and-suggestions**, oder schreibe "Flash Galatine" eine DM
3. Gib deine Discord-Benutzer-ID und den Namen des Presets an

Widersprüche werden innerhalb von 7 Tagen geprüft. Um deinen Kontodatensatz und alles, was darunter
eingereicht wurde, entfernen zu lassen, siehe [`PRIVACY.md`](PRIVACY.md).

## Faire Nutzung der Website

Bitte nicht:

- Ratenbegrenzungen umgehen oder die Website so automatisieren, dass sie für andere Nutzer
  schlechter wird
- die Website oder die API über das hinaus scrapen, was die
  [Entwicklerdokumentation](https://developers.xivdyetools.app) erlaubt
- versuchen, irgendeinen Teil des Dienstes zu brechen, zu untersuchen oder unbefugten Zugriff darauf
  zu erlangen
- die Website nutzen, um jemanden zu belästigen oder Malware zu verbreiten

Wenn du ein Sicherheitsproblem findest, melde es bitte privat per E-Mail, statt es an der
Live-Website vorzuführen.

## Dienste anderer Anbieter

Manche Funktionen leiten dich an Dienste weiter, die wir nicht selbst betreiben:

- **Universalis** liefert Marktbrett-Preise.
- **Discord** und **XIVAuth** übernehmen die Anmeldung; Discord hostet außerdem unseren
  Community-Server.
- Die **Gegenstands-Links** bei einem Mirage-Teil öffnen diesen Gegenstand bei Garland Tools,
  FFXIV Teamcraft, Gamer Escape, Mirapri, Saddlebag Exchange oder dem Lodestone.

Sobald du einem dieser Links folgst, befindest du dich auf der Website einer anderen Partei, unter
deren Bedingungen und deren Datenschutzrichtlinie. Wir kontrollieren sie nicht und sind nicht für
sie verantwortlich.

## Material von Square Enix

FINAL FANTASY XIV © SQUARE ENIX CO., LTD. Alle Rechte vorbehalten. FINAL FANTASY ist eine
eingetragene Marke der Square Enix Holdings Co., Ltd.

Dies ist ein Fan-Projekt und steht **in keiner Verbindung zu Square Enix Co., Ltd. und wird von
diesem weder unterstützt noch gesponsert.** Spieldaten — Farbstoffnamen, Farben,
Erwerbsmethoden, Gegenstandsnamen und -symbole — gehören Square Enix und werden hier zu
Informationszwecken verwendet. Der eigene Code, das Design und die geschriebenen Inhalte der
Website gehören uns.

## Alter

Die Website ist für Personen gedacht, die mindestens 13 Jahre alt sind, oder das in deinem Land
vorgeschriebene Mindestalter, je nachdem, welches höher ist. Die Anmeldung erfordert ein
Discord- oder XIVAuth-Konto, jeweils mit eigenem Mindestalter. Wenn du glaubst, dass jemand unter
diesem Alter uns Daten überlassen hat, kontaktiere uns, und wir werden sie entfernen.

## Verfügbarkeit

Dies ist ein Hobbyprojekt, das auf unsere eigenen Kosten betrieben wird. Wir können jeden Teil
davon jederzeit ändern, unterbrechen, verschieben oder abschalten, mit oder ohne Ankündigung.
Paletten und Sammlungen, die du speicherst, liegen in deinem eigenen Browser — löschst du deine
Website-Daten, sind sie weg, und wir können sie nicht für dich wiederherstellen. Alles, worüber du
verärgert wärst, es zu verlieren, ist es wert, exportiert zu werden.

## Keine Gewährleistung

DIE WEBSITE WIRD "WIE BESEHEN" UND "WIE VERFÜGBAR" BEREITGESTELLT, OHNE GEWÄHRLEISTUNG JEGLICHER
ART, AUSDRÜCKLICH ODER STILLSCHWEIGEND, EINSCHLIESSLICH JEGLICHER STILLSCHWEIGENDEN GEWÄHRLEISTUNG
DER MARKTGÄNGIGKEIT, DER EIGNUNG FÜR EINEN BESTIMMTEN ZWECK UND DER NICHTVERLETZUNG VON RECHTEN
DRITTER.

Einfacher ausgedrückt: Wir versprechen nicht, dass die Website erreichbar bleibt, dass
Farbabgleiche oder Marktpreise genau sind, oder dass sie in jedem Browser funktioniert.
Farbstoff-Farben sind Annäherungen daran, wie ein Farbstoff im Spiel dargestellt wird, und
Marktpreise stammen von einem Drittanbieter und veralten.

## Haftungsbeschränkung

SOWEIT GESETZLICH HÖCHSTZULÄSSIG, HAFTEN WIR NICHT FÜR INDIREKTE, ZUFÄLLIGE, BESONDERE,
FOLGE- ODER STRAFSCHÄDEN ODER FÜR JEGLICHEN VERLUST VON DATEN, GEWINNEN ODER GIL, DIE AUS DEINER
NUTZUNG DER WEBSITE ENTSTEHEN.

Manche Rechtsordnungen erlauben diese Beschränkungen nicht; in diesem Fall gelten sie für dich nur
so weit, wie es das Gesetz zulässt.

## Änderungen dieser Bedingungen

Wir können diese Bedingungen aktualisieren. Änderungen werden hier mit einem neuen Datum
"Zuletzt aktualisiert" veröffentlicht, und bedeutende Änderungen werden auf dem Discord-Server
angekündigt. Die fortgesetzte Nutzung der Website nach einer Änderung bedeutet, dass du sie
akzeptierst.

## Beenden

Du kannst die Nutzung der Website jederzeit beenden. Wir können deinen Zugang zu den
Community-Funktionen sperren oder beenden, wenn du gegen diese Bedingungen verstößt oder den
Dienst für andere Nutzer störst. Presets, die du veröffentlicht hast, können auch nach deinem
Weggang unter der oben genannten Lizenz bestehen bleiben — frag uns, wenn du möchtest, dass sie
entfernt werden.

## Anwendbares Recht

Diese Bedingungen unterliegen dem Recht des Staates North Carolina, USA, ohne Anwendung seiner
Kollisionsnormen. Jeder Streit, der nicht zunächst im Gespräch beigelegt werden kann, wird vor die
staatlichen oder bundesstaatlichen Gerichte in North Carolina gebracht, und du und wir stimmen
diesem Gerichtsstand zu. Nichts hierin nimmt dir ein Verbraucherschutzrecht, das dir dein
örtliches Recht gewährt und das du nicht abbedingen kannst.

## Kontakt

- **E-Mail**: FlashGalatineFGC@gmail.com (Betreff: "XIV Dye Tools Terms")
- **Discord**: <https://discord.gg/rzxDHNr6Wv>
- **Support-Kanal**: #dyetools-issues-and-suggestions
- **Issues**: <https://github.com/FlashGalatine/xivdyetools/issues>
