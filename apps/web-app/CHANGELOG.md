...
- **`en` locale values now use American spelling** (41 keys across `accessibility.*`,
  `comparison.*`, `swatch.*`, `budget.*`, `preset.*`, `share.*`, `welcome.*`, `config.*`,
  `harmony.*` and `advanced.behaviorTitle`), plus the two share-URL validation errors in
  `src/services/share-service.ts`. Keys are untouched — `advanced.behaviorTitle` was already an
  American key holding a British value.
- **Documents quoting those labels were updated with them** — the user guides, `docs/projects/web-app/tools.md`,
  `docs/specifications/collections.md` and this file's own 16A entry all named *Save character colours*
  and *Behaviour toggles*, which the app no longer says.
...
- **Every modal opened underneath the app chrome.** `.m16-backdrop` was `z-index: 50` while the app
  bar, palette drawer, Options panel and both corner FABs sit at 90-100 (result-card tooltips at
  200). The shell host creates no stacking context, so its `position: fixed` children compete with
  `#modal-root` directly and won. The visible symptom: on mobile the palette FAB covered the welcome
  modal's "Get started" button and swallowed taps on the overlapping strip — Playwright reported
  `<v4-layout-shell> intercepts pointer events`. Three places set a modal z-index (the class rule,
...
- **A `--v4-z-modal` / `--v4-z-toast` scale in `themes.css`** makes the stacking order explicit.
  Chrome stays at 90-200, modals move to 1000, toasts to 1100, and the tutorial spotlight keeps the
  top of the stack at 9998-10000 (it points *at* the chrome, so it must outrank all of it).
...
- **Loading the app no longer opens a connection to `universalis.app`** (`docs/audits/2026-08-29-security` FINDING-026). `src/index.html` carried `dns-prefetch` and `preconnect` hints for `https://universalis.app`, so every page load handed a third party the visitor's IP address and the site name in the TLS handshake — for a connection no request ever used, because market prices have gone through the first-party proxy at `data.xivdyetools.app` since api-worker absorbed it, and because a resource hint is not a fetch the CSP is ever consulted about. Both hints are gone, and `https://universalis.app` is out of the CSP's `connect-src`, which now names only `'self'` and `https://*.xivdyetools.app` — matching what `PRIVACY.md` already told people. `src/__tests__/security-headers.test.ts` now rejects any third-party host in either place, so neither can come back quietly. Unaffected: the Universalis credit in the About modal and the "view this item's market listing" link on a result card — those are links a person clicks, not connections the page opens.
...
- **The Advanced Options "Enable Analytics" switch is real** (`src/services/telemetry-service.ts`; spec `docs/superpowers/specs/2026-08-29-web-analytics-design.md`). Off by default, and off regardless of the switch when the browser sends Global Privacy Control. When on, the app beacons small `text/plain` batches to api-worker's `POST /v1/telemetry` (Analytics Engine): tool views with an `entry` of `initial` / `share` / `nav` and visible-seconds dwell, accepted explicit dye picks (palette drawer or dye grid / Favorites strip — never random buttons, removals or picks the tool dropped), `.chara` parse outcome + producer family, and deliberate theme switches (theme modal and Shift+T, via the new `services/theme-switch.ts`). Envelope: version, env, UI language, theme, viewport bucket. No identifiers, no storage, nothing persisted. Switching it off — in any tab — drops the queue at once (`ConfigController` now re-reads a config another tab saved). The pre-5.x `ShareService` localStorage analytics buffer is retired and its key cleaned up.
- **`apps/web-app/PRIVACY.md`** — the web app's privacy guide now lives in this repo (the extractor's "Privacy Protected · Learn more" link pointed at the archived repo's copy, which stated no telemetry exists). It documents every network call the 5.0 app makes and the opt-in analytics above.

### Fixed — 2026-08-29

- **Clicking a dye card's add-to-collection button now opens the collection menu** instead of selecting the dye: the card's click handler stopped propagation before the delegated wrapper handler could see the click.
...
- **Palette drawer category headings were English in every locale** — the drawer's own `CATEGORY_TRANSLATION_KEYS` map never matched the runtime category strings core actually produces, so every heading (Whites, Grays, Blacks, Browns, Reds…) silently rendered in English regardless of language. It now renders through `LanguageService.getCategory()` and takes its section order from core's category list; the eleven now-dead `colorPalette.*` keys were deleted from all six locales
- **Community Presets surface was largely un-localized**: the edit form, the submission form, every validation toast, vote/collection error toasts, and the login-gate prompts render through `LanguageService.t()`/`tInterpolate()` now instead of English literals. `preset-submission-service.ts`, `community-preset-service.ts` and `collection-service.ts` stopped returning English error strings and return typed codes instead — the UI maps each code to a locale key, so a network failure, a validation failure and a duplicate-vote failure each get their own translated toast instead of one English fallback
- **Route titles and tool-load messages were English-only** — `router-service.ts`'s `ROUTES[].title` now resolves through a `titleKey`, `document.title` updates on every route change and on locale switch, and the "Loading…" / tool-load-failure strings in `v4-layout.ts` are keyed
- **Five components went stale after a language switch**: `v4/result-card.ts`, `v4/v4-layout-shell.ts`, `v4/display-options-v4.ts`, `v4/dye-filters-v4.ts` and `v4/share-button.ts` never subscribed to `LanguageService`, so text on screen kept reading the previous language until the next unrelated re-render. All five now subscribe and unsubscribe correctly; a new locale-switch test (`src/__tests__/locale-switch.test.ts`) mounts each `components/v4/` component, flips the locale, and asserts the rendered text actually changed, pinning the regression
- **Harmony/vision/category vocabulary now has one source, not two**: the config sidebar's harmony-type `<option>` labels and the vision-toggle labels read from core's `getHarmonyType()` / vision vocabulary instead of a separate, drifting `config.*` translation table; the ten now-redundant `config.{complementary,analogous,…,invertedTetradic}` keys were deleted from all six locales
- **Custom-dye names, `Gil` formatting and dye names in search/sort/aria** now go through the shared `shared/custom-dye.ts` (`makeCustomDye()`), `shared/format.ts` (`formatGil()`, `formatNumber()`, `formatDate()`) and `shared/dye-name.ts` (`localizedDyeName()`, `compareDyeNames()`, `dyeNameMatches()`) helpers instead of seven separate ad-hoc `Custom (#HEX)` literals and hand-rolled `toLocaleString()` calls scattered across the nine tools — result cards, budget, mixer, comparison, dye search, dye sort and aria labels all read the localized name and the locale-formatted number now
- Imported collections' generated copy names are localized: what used to always read `"{name} (imported {n})"` in English now interpolates through a locale key, so a ja/de/fr/ko/zh user sees the suffix in their own language
- Result cards' gil readout changed from a bare `1,234 G` to the fully-localized `1,234 Gil` (and the equivalent in each of the other five languages) via `formatGil()`
- Budget, mixer and gradient export-meta lines (the small caption under an exported palette naming the matching method/model) are now generated from locale keys instead of concatenated English fragments
...
- **Per-locale CJK font order** (`globals.css`): `--font-cjk` now has `:root:lang(zh)` and `:root:lang(ko)` overrides that put Noto Sans SC / Noto Sans KR ahead of Noto Sans JP, so a zh or ko reader with multiple CJK families installed no longer sees Han characters drawn with Japanese glyph forms; `my-submissions-modal.ts` no longer inlines `font-family: 'Fragment Mono'` (four sites) and instead uses the shared `var(--font-mono)` contract, restoring its CJK fallback chain
- **i18n guardrails, new**: `validate:i18n` now also asserts the five non-English locale files match `en.json`'s key order and carry no stray whitespace drift, and runs a promoted `scripts/i18n-parity.mjs` (duplicate/interpolation/identical-to-EN checks with an allow-list) as part of the same command; a new ESLint rule (`xivdyetools-i18n/no-hardcoded-ui-strings`, in `eslint-rules/`) flags sentence-shaped English string literals reaching the DOM outside `LanguageService.t*()` — `npm run lint` is warning-free with the sole brand-name exception (`result-card.ts`'s "Saddlebag Exchange" link, rule-scoped disabled)
- **Pages smoke test waits up to ~2 min for the deployment alias** (`scripts/smoke-test-pages.js` `REACH_ATTEMPTS` 6 → 24): the 2026-08-21 beta deploy of the i18n merge (`8add1630`) answered 404 for 25 s+ after `wrangler pages deploy` returned and then served normally, so the old ~25 s budget red-flagged a deploy that had worked. The custom-domain convergence budget (~175 s) and the robots assertion are unchanged
...
**Operator / deploy-day (web-app deploys from `main` via `deploy-web-app.yml`; the beta app from any non-main branch via `deploy-web-app-beta.yml`):**

- Deploy the branch's **api-worker** with (or before) the web app — the market board is hard-down (404) against the currently deployed worker because `data.xivdyetools.app/universalis/*` does not exist there yet. Do **not** repoint at `proxy.xivdyetools.app` as a stopgap (its CORS header is pinned to the production origin)
- Deploy **presets-api** and run its D1 migrations first (`0007_drop_community_category`, the generated stainID rewrite from `scripts/migrate-dyes-to-stainids.ts`, `0008_add_example_link`, `0009_add_preview_image`, `0010_add_secondary_categories`) — until the stainID rewrite runs, live community rows still carry legacy itemIDs (the client's `resolvePresetDye()` routes by ID range so they keep rendering, but rail counts skip the legacy `community` category)
- The **oauth** and **presets-api** allowlists must include `https://beta.xivdyetools.app` before the first beta push, or beta login / community presets fail; the beta Pages project must be deployed with `--branch=beta` (any other branch name is a silent preview)
- The root social cards live at fixed URLs (`/og/default.png`, `/og/default-x.png`) with a 1 h edge TTL — a copy cached under the previous 7 d TTL needs a manual purge
...
- **Swatch — 10A sheet + `.chara` import** ([`src/components/swatch-tool.ts`](src/components/swatch-tool.ts), new [`chara-import.ts`](src/components/chara-import.ts)): drop an Anamnesis / Ktisis `.chara` file — parsed entirely on-device (core's `parseCharaFile` → `resolveCharaColors`) into a file card (producer, nickname, LOCAL ONLY chip, tribe/gender readout), a THIS CHARACTER sheet (one card per slot with R·C grid address or amber OFF GRID, absent-slot reasons, best dye + tier-colored ΔE2000, lip blend rendered beside the raw cell), grid pins on the loaded palette, a five-row excerpt around a picked cell, and DYES ON THIS GLAMOUR (both channels, droppable chips, 3–6 counter → prefilled preset submission); **Save character colors** (`kind: 'character'` record); seven-palette rail + Dark/Light range toggle on the grid (replacing the sidebar dropdown), race/gender readout lock when a file is loaded, SEND TO handoff row, share grammar keyed on cell address, 26 px desktop / 44 px mobile cells; **Evercold deprecation banner** on the eye / hair / skin grids (`EVERCOLD_DEPRECATED_CATEGORIES`, `tools.character.evercoldNotice*` ×6); `swatch.*` net +79 keys ×6; FR tool name becomes *Nuancier*, extractor title/shortName retranslated ×6
...
- **Misc**: `README.md` for the app; `scripts/README.md`; `e2e/COVERAGE-GAPS.md`; `wrangler` declared as a devDependency so `wrangler-action` stops mutating `package.json` mid-deploy; `lint` is read-only (`lint:fix` is explicit)
...
- **Themes**: the ten retired themes (`themes.css` blocks, `sugar-riot` special cases, `themes.*` locale names)
...
- **DEAD-037 (Wave 4a, adopt)**: `shared/constants.ts`'s `SUPPORTED_LOCALES` now reads `@xivdyetools/core`'s constant directly instead of holding its own byte-identical literal copy; `LOCALE_DISPLAY_INFO` (the richer name/flag shape) is unchanged, and a new `shared/__tests__/constants.test.ts` parity test pins both the equality with core's list and the 1:1 code/order match against `LOCALE_DISPLAY_INFO`.
- **DEAD-037 (Wave 4a)**: `commands/accessibility-tool.ts`'s local `VISION_TYPES` display list gains a compile-time bidirectional check against `@xivdyetools/types`' `VisionType` union (errors if either side gains or drops a lens the other doesn't know about) rather than being merged into one shared table — the two lists carry different per-app shapes (this one includes UI prevalence/description strings) and were never provably the same shape to begin with.
- **4th missed `#CE2222`/`#EA4133` site** (DEAD-018 follow-up, Wave 4a): `budget-tool.ts`'s `accent()` hard-coded both the light and dark literals instead of reading `@xivdyetools/svg`'s `GLYPH_ACCENT_LIGHT` / `GLYPH_ACCENT_DARK` — same values, now single-sourced like the other three sites.
...
- **Beta / deploy guards**: the beta deploy workflow runs `check-beta-build` so a mis-set `VITE_APP_ENV` fails the job instead of publishing an indexable copy of production; the beta plugin's idempotency guard was defeated by a comment naming the header (`hasBetaHeadersBlock()` now matches an indented directive); the social-card edge TTL was borrowed from og-worker's immutable-per-URL case and left the redesigned root card stale for a week
- **Audit remediation** (2026-08-09 pre-release audit, `docs/audits/2026-08-09-prerelease-monorepo-upgrade/`): BUG-002 / BUG-003, DEAD-001…DEAD-007, HC-001…HC-004, REFACTOR-001…REFACTOR-005, FINDING-004 (dev-toolchain advisories: `postcss` ^8.5.26, `wrangler` ^4.120.0 lockfile refresh — 14 advisories → 6, the rest are `vitepress`/`tsup` transitive with no fix)
...
- Vitest coverage is **actually gated** now — `vitest.config.ts` declared thresholds in the removed Vitest 1.x shape, so the suite had had no coverage gate; the honest baseline was 50.9 %, ratcheted commit-by-commit to **71 / 55 / 65 / 72** (statements / branches / functions / lines) with the remaining gap named in a comment. The eight tool suites were unblocked (a swallowed `safeRender()` throw from any missing mock method rendered nothing and let assertions pass on an empty DOM): icon modules unmocked, `@xivdyetools/core` mocked with `importOriginal`, service mocks completed against the real APIs, real `harmony-generator` / `mixer-blending-engine` via `importActual`, a decoded-image + 2D-context fake for the extractor; per-tool: accessibility 21 → 71 %, harmony 24 → 65 %, mixer 28 → 70 %, gradient 32 → 64 %, extractor 19 → 84 %, swatch 16 → 67 %, comparison 24 → 76 %, `image-zoom-controller` un-excluded (38 → 78 %). Vacuous assertions (`if (x.length === 0) return`, constant-valued distance mocks that flipped the whole budget ledger on/off) were removed rather than kept green
- New suites: `font-contract`, `bundle-budget` (32), `beta-branding`, `router-service-title`, `v4-app-header`, `v4-layout-shell`, `theme-service` legacy-migration (10 cases), `collection-service` 5.0 migrations, `metric-help`, `preset-submission-form`, `preset-category-selector`, `smoke-test-pages` arg parsing, `changelog-parser`; announcer-service and rAF-dependent extractor tests de-flaked (poll / `vi.waitFor` on the signal, not fixed sleeps); the k-d-tree perf benchmark now times the path it claims to (ratio, not absolute)
- **E2E**: rewritten for the 5.0 shell — shared `e2e/fixtures/navigation.ts` (`waitForAppReady`, `gotoTool`, `switchToolViaMenu`, `seedStartupStorage` reads the real version so What's New can't reappear, `dismissBlockingOverlays` knows `.m16-backdrop`, `closePaletteDrawer`, viewport-agnostic `toolSwitcher` / `activeToolControl`); ui-interactions rewritten against the 5.0 DOM (21 real tests replacing 16 silent skips); preset-browser assertions branch on the documented offline state; the coverage rig (`chromium-coverage`) now wraps all specs instead of one. `--project=chromium` 142/142 green; **`mobile-chrome` has 28 pre-existing failures** (the tools' left-panel controls are never mounted at phone width — a product gap, documented with the parked fix in `e2e/COVERAGE-GAPS.md`)

### Documentation

- `CLAUDE.md` resynced (125-dye database, two themes, font contract, `publicDir` rule, beta build, Universalis via api-worker); per-tool port specs under `docs/research/monorepo-2.0/` (1A dial, 3C loupe, 4C pin rail, 5C mixing field, 6A lens, 7C duel, 8A gallery, 9C ledger, 10A sheet, 2.6g result card / empty states); `docs/operations/DEPLOY_ENVIRONMENTS.md` gains the beta web app; READMEs audited (Blog link dropped, MIT + Square Enix attribution everywhere)
...
- **Budget Suggestions — matching algorithm control**: Added the shared matching-method selector to the Budget section in [`src/components/v4/config-sidebar.ts`](src/components/v4/config-sidebar.ts) and wired it through `BudgetConfig` so the selected algorithm now drives Budget candidate lookup and Delta-E calculations for sorting/cards.
