...
- **Fatal-error fallback works under the CSP (WEB-9).** The screen main.ts paints when service initialisation throws is built with DOM APIs in `shared/fatal-error.ts`; its Reload button had an inline `onclick`, which `script-src 'self'` blocks — it did nothing in production.
