...
- **`/og/harmony/*` and its page share URLs carry `?wheel=`** — allowlisted against
  ...
  is (400 on an unrecognised spelling), cache-keyed alongside `lang`/`frame`/`algo` so
  the five wheels of one dye+harmony each buy their own edge-cache entry, and elided
  from the URL when it is the default `rgb` wheel so an ordinary share link's cache key
  is unchanged from 2.9.0. The deck names the wheel when it is not the default.
...
- **OG-9 — unknown paths under the routed prefixes answer crawlers with `404` + `Cache-Control: no-store`** (still the fallback embed in the body) instead of a cacheable `200`.
- **OG-8 — `400` bodies no longer echo the offending parameter** (`Invalid harmony type` / `Invalid algorithm` / `Invalid vision type`).
- **OG-7 — observability footprint**: the logger middleware no longer records the raw User-Agent of every request (`logUserAgent: false`); crawler hits log theirs in the `Serving OG metadata` line (the string worth having when a new crawler needs a detector pattern). `og_request` Analytics Engine datapoints are written for **crawler hits only** — human page views on the nine production tool paths no longer produce a datapoint (they were cost, not signal; a dashboard counting `crawler = none` rows will see them stop at this deploy).

### Added

- **Edge cache for rendered PNGs** (`index.ts`): `/og/*` GET 200s are stored in `caches.default` keyed on the full URL (lang / frame / algo all vary the image) and served from the cache on repeat requests; the `Cache-Control` / `CDN-Cache-Control` headers renderOGImage sets were inert on their own, so every hit was a full resvg raster. Error responses are not cached; outside Workers (tests) the middleware is a pass-through. (FINDING-005)
