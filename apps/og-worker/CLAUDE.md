...
└── services/
    ...
    └── svg/
        ...
        ├── base.ts             # Primitives re-exported from @xivdyetools/svg
...
| `GET /og/swatch/:color/:limit[.png]` | `?algo=`; `limit` 1–20, capped to 4. `?sheet=`/`?race=`/`?gender=` are parsed but the 15E card does not use them |
...
| `GET /og/extractor/:colors[.png]` | `RRGGBB-share` pairs, comma-separated, max 5 |
