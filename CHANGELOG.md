...
- **`docs/` reorganised.** `docs/brainstorming/` is retired — all 18 files described shipped or
  ...
  was deleted in favor of `docs/historical/index.md`. `docs/README.md` is now a short folder
...
- **`docs/index.md`** carries a September 2026 "Recent Updates" section (harmony convergence,
  ...
  color wheels) and no longer describes the August wave as pending.
...
- The findings themselves — wrong OAuth callback flow and vote API in the architecture pages,
  ...
  at a "report" button and a footer link that are not there — are itemized per sweep in
...
- **The dead-code gate could exit 0 without running.** `scripts/check-dead-code.ts` decided whether
  ...
  `isMainModule`, which `realpathSync.native` also uses to normalize drive-letter case on Windows.
...
- **The Playwright suite runs in CI, in its own `e2e` job — until now it ran in no workflow at
  ...
  green-by-absence: `ca0ee36d` (an i18n grammar fix that changed the palette drawer's favorites
...
- **stainID everywhere**: share URLs (`?dye=<stainID>` + the `?hex=` bare-color grammar; legacy itemID links are rejected loudly), og-worker paths, `presets.json` 2.0.0 (15 curated rows), community presets in D1 (`scripts/migrate-dyes-to-stainids.ts` + migration 0007), the web-app's collections/favorites (`CollectionService` 5.0, exact disjoint-range migration on load), and the Discord dye-emoji set (regenerated from `dyes.json`, user-run re-upload).
- **One matching vocabulary** across core, web-app, bot, og-worker and api-worker: `ciede2000` (new default) / `oklab` / `cie76` / `redmean` / `rgb` / `distinguish`; `hyab` and `oklch-weighted` retired, stored values normalize on read via `normalizeMatchingMethod`; per-method calibrated quality bands (`classifyBandTier`) replace four divergent threshold copies.
...
| `@xivdyetools/logger` | 2.1.1 | throw-safe serialization + broader redaction (2.1.0); then the 2026-08-29 audit — value-shape redaction reaches string array items and free text, aliased values are redacted at every reference, and serialization is bounded (2.1.1) |
