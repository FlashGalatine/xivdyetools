...
- **Carried forward** (`POST_MERGE_CHECKLIST.md` §3/§5): remove v1 bot-HMAC acceptance and the KV rate-limiter fallbacks once production confirms the new paths; GitHub secret scanning + push protection and the `production` environment rules are repo settings; dev-only `vite`/`esbuild` advisories (FINDING-036).
...
| `api-worker` | 0.7.0 | absorbed universalis-proxy + api-docs, schema v2 serving, 5.0 matching vocabulary; `/v1/chara/resolve` + `/v1/chara/icon/:id` (.chara equipment identity, ko/zh name tables, CORS +POST) — **deployed to production by hand 2026-08-21** |
