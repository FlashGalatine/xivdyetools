...
og-worker, oauth and presets-api behaviour, the bot policy describes presets-api and Perspective —
...
judgement call made while editing:
...
2. **Code collects something undisclosed that is genuinely necessary** → still a finding against
   the code (MEDIUM, exposure of the unit). Fix = **minimise first, then disclose**: cut the field
