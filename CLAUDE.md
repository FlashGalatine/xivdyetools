...
- **knip 6** for dead code. The root `knip.jsonc` is the monorepo graph; `packages/{core,svg,bot-logic}` are gated on it via their own `lint:dead` (`knip --directory ../.. --workspace packages/<p>`, part of `lint`), while `apps/web-app` and `apps/og-worker` keep their own per-app `knip.jsonc`. Those three packages run with `includeEntryExports`, so a **barrel export nothing in the monorepo imports fails `lint` unless its specifier carries a `/** @public */` tag** (`"tags": ["-public"]`) — `@public` meaning "published npm API, deliberately kept without an in-repo consumer". All seven apps except `stoat-worker` are now gated via their own `lint:dead` scripts. `pnpm lint:dead` at the root sweeps every workspace; as of 2026-09-01, it reports 4 known-and-documented findings (2 unused `.d.ts` false positives, 1 kept `wrangler` devDependency in web-app, 1 duplicate export in logger) from the five ungated packages (`auth`, `logger`, `types`, `worker-kit`, `test-utils`). Until those packages are gated, the root sweep serves as a reference point, not a gate; the per-package `lint` commands above are the actual gate.
