...
Short form of `.agents/skills/audit-shared/traps/git-worktrees-and-finishing.md`; read that file before worktree, verification, or branch-finishing work. The project skills are tracked in `.agents/skills/` (vendor-neutral; `.claude/skills` is a tracked symlink to it, and `AGENTS.md` is the entry point for non-Claude agents) — edit them there, and see `.agents/skills/audit-shared/README.md` § *Discovery* for the Windows symlink and parent-folder caveats.

- **Worktrees** go in `.claude/worktrees/<name>` (the runtime's native worktree tool, or `git worktree add`; ignored via `.gitignore`). Each needs its own `pnpm install --frozen-lockfile`. `git worktree remove` half-deletes on Windows unless `core.longpaths=true` is set (it is here); purge leftovers with robocopy, then `git worktree prune`. Another session usually has this checkout open: never `git stash`, never switch branches here, and commit with `git commit --only -- <paths>`.
- **The gate** is `pnpm turbo run build type-check lint test` (`--filter=...<unit>` — leading dots — for a unit and its dependents; `<unit>...` with trailing dots selects its dependencies instead). It hides more than tests: knip inside `lint` for web-app / og-worker / core / svg / bot-logic, the no-hardcoded-UI-strings ESLint rule, i18n orphan + public-metadata vitest files, the locale-quality gates (web-app `scripts/i18n-parity-gate.test.js`, bot-logic `src/i18n/__tests__/locale-quality.test.ts` — key parity, `{placeholders}`, values left in English, **same English → same translation**; exceptions need a reason in the allow-list beside each, and a stale exception fails), and the CJK `font-coverage` / `font-faces` tests in discord-worker and og-worker. Not inside it: web-app's bundle budget (`run build:check`), the wrangler-invariants step (CI only), and gitleaks. `validate:i18n` itself still runs in no workflow — since 2026-09-20 the parity-gate test runs both of its scripts, which is what makes them a gate. Single test file: `pnpm --filter <pkg> exec vitest run <file> --coverage.enabled=false` (without the flag a green subset run exits 1 on coverage thresholds) — never `npm test <path>`.
...
- **Turborepo 2.11** for task orchestration with dependency-aware caching
- **TypeScript 5.9** with shared `tsconfig.base.json` (strict, ES2022, bundler resolution, `verbatimModuleSyntax`)
- **Vitest 5** for all packages and apps; **Playwright** for `web-app` E2E
- **ESLint 10** flat config with typescript-eslint
- **Prettier 3** for formatting
- **knip 6** for dead code. The root `knip.jsonc` is the monorepo graph; every package under `packages/*` is gated on it via its own `lint:dead` (`knip --directory ../.. --workspace packages/<p>`, part of `lint`) — `core`/`svg`/`bot-logic` first, then `types`/`worker-kit`/`auth`/`logger`/`test-utils` joined in Phase 3 (2026-09-01) — while `apps/web-app` and `apps/og-worker` keep their own per-app `knip.jsonc`. All eight packages run with `includeEntryExports`, so a **barrel export nothing in the monorepo imports fails `lint` unless its specifier carries a `/** @public */` tag** — `@public` meaning "published npm API, deliberately kept without an in-repo consumer" (the workspace-private `test-utils` is the one package that deletes an unused export instead of tagging it, since it has no npm consumers to break). That exemption comes from knip's built-in `isAlwaysIgnored`, **not** from the `"tags": ["-public"]` line in `knip.jsonc`, which is inert (removing it, keeping it, or inverting it to `+public` all give byte-identical output, and `--tags=+public` cannot enumerate the tagged symbols). The same built-in also exempts `@beta` and `@alias`, which this repo does not use — `pnpm dead-code:check` fails on either so that undocumented escape hatch cannot be taken by accident. Eight of the nine apps (all but the parked `stoat-worker`) are also gated via their own `lint:dead` scripts. `pnpm lint:dead` at the root sweeps every workspace; as of 2026-09-01 (post-Phase 3), every package and gated app is clean and it reports only 3 known-and-documented findings, all from `apps/web-app` (2 unused `.d.ts` files + 1 kept `wrangler` devDependency, all ignored by web-app's local `knip.jsonc` but visible at root due to config scope divergence). Every workspace except `apps/stoat-worker` (parked) is now gated, so the per-package/per-app `lint` commands are the actual gate and the root sweep is a cross-check that should stay at exactly those 3.
- **Dead-code reachability gate** (`pnpm dead-code:check` → `scripts/check-dead-code.ts`, self-tested by `pnpm test:scripts`; both run in CI immediately after `Type-check (affected)`, alongside a permanent `pnpm type-check:scripts` for the `scripts/` directory itself) closes a gap knip cannot: modules, exports, and **class members** reachable only from test files. knip treats every test file as an entry, so anything a test imports counts as used, and knip 6 dropped its `classMembers` rule entirely — a dead public method is invisible to it no matter how it's configured. Three exemption tags, asymmetric on purpose: `@testonly <reason>` (only tests reach this; it may be deletable) and `@entrypoint <reason>` (reached only by an external convention static analysis can't see — Pages `functions/`, a CLI script run from `package.json`; must never be deleted) both require a reason, a bare tag fails the gate, while `@public` (published `@xivdyetools/*` API with no in-repo consumer) does not. `@beta` and `@alias` are **rejected outright**: knip hard-codes all three of `@public`/`@beta`/`@alias` as always-ignored, so either of the latter two would silently exempt a dead export with no review signal, and only `@public` is a convention here.
  ...
  - Regex literals are never masked (telling a regex `/` from division needs the preceding token, a parser's job), so a quote or backtick inside one throws the span tracker off — a backtick opens a template that runs to end of file, and a quote can swallow a real backtick later on the same line with the same effect; either way the checker falls back to raw text for candidacy on that whole file — over-reports, but bounded to that file. Recount at any time: over `listTracked()`, a file where `declarationLines(text)` differs from `maskSource(text).split('\n')` fell back (3 differ at HEAD). It cuts the other way at **member** granularity, where `attributeLinesToBlockFrames` deliberately takes no such fallback: one of those regexes inside a *method body* freezes the brace depth one level below the class's own, every later line in the file then reads as nested rather than direct, and every member declared after it — in that class and in every class below it — is silently dropped (masked lines are blank, so the column-0 resync never fires either). Under-reports, bounded to that file. The same regex sitting at the class's own body depth instead keeps the members but misattributes every later one to that class.
  ...
  - `apps/stoat-worker` is filtered out of `listTracked()` wholesale, so it is neither a production nor a test referrer. A package export whose only real consumer is that app therefore looks unreferenced — `resolveDyeInput` (`packages/bot-logic`) is exactly this case today, and survives only because a *comment* in `apps/discord-worker/src/index.ts` happens to name it. Rewording that sentence would make a live export report as dead. Before deleting anything the gate flags in `packages/bot-logic`, grep `apps/stoat-worker` for it by hand.
  - Path-alias imports (`@shared/…`, `@services/…`) are resolved against the importing workspace's own `compilerOptions.paths` (`resolveAliasSpecifier`). Before 2026-09-02 they fell through to filename-only matching that crossed workspace boundaries — web-app's `@shared/logger` also matched `packages/worker-kit/src/middleware/logger.ts` — which under-reported. A specifier whose workspace declares no matching alias still takes that basename fallback, so a bare package name resolves as before.
  - Several declaration shapes are not candidates in the first place, so a dead one is never reported (all under-report, all verified against the regexes): `export abstract class`, `export enum` and `export default` are not `EXPORT_DECL` matches, which only accepts `export [async] function|const|let|class Name` — an `export default class` is not even a `CLASS_DECL`, so it does not open its file for member scanning at all; `override` methods, `abstract`/`declare` method signatures, and arrow-function class fields (`handler = (): void => {`, common in Lit) are not `MEMBER_DECL` matches, which needs `name(` after only `public|static|async|get|set`; and an `index.ts` is never an orphan-module candidate by construction.
