...
- **Five selectable harmony colour wheels** — `COLOR_WHEEL_IDS`, `DEFAULT_COLOR_WHEEL`,
  ...
  5.1.0 (the golden digest is unchanged). The Munsell hue table was cross-checked
...
- `ColorWheel.carriesBaseHsv` — false for `oklch-lightness`, whose target keeps the base's
  ...
  ranking scores in degrees against a target hue and so discards the very lightness the
  wheel exists to hold, returning a partner at some other lightness entirely. Every other
  wheel is unaffected, and the `hue-only` golden digest does not move.
- `ColorConverter.gamutMapOklch` (CSS Color 4 §14 binary search with local MINDE) and
  ...
  hue back in sRGB gamut without clipping. Measured against culori as an external
  oracle, `gamutMapOklch` has a **0.975 exact-match rate**. `maxChromaOklch` answers the
  largest C such that **every** chroma in `[0, C]` is inside sRGB — it scans outward from
  the neutral axis and bisects the first exit, rather than bisecting a fixed range. The
  sRGB solid is not connected along the blue ray, so a plain bisection could land in the
  far lobe that makes `#0000FF` legal and answer a chroma with an out-of-gamut hole
  beneath it (at L≈0.11 / h≈264° it returned 0.076 where the connected answer is 0.066).
...
- **The wheel tables are committed DATA, not import-time derivation.** The OKLCH-hue table
  is generated by `scripts/build-oklch-hue-table.ts` (`pnpm run build:oklch-hue`, not part
  ...
  `oklch-hue` per-wheel digest is re-baselined `c927b9fb…` → `d3e36ef2…`; the RGB golden
  digest and the other three wheels are unchanged.
- The Munsell generator writes two files: `src/data/munsell-hues.json` (the table the wheel
...
- `HarmonyColorSpace`, `HarmonyOptions.colorSpace`, and `HarmonyGenerator.rotateHueInSpace`
  are **deprecated** — they rotate hue without carrying the base's saturation and value.
  The new `wheel` option on `HarmonySelectionConfig` replaces them; nothing in the monorepo
  still calls the deprecated path after this release. Deprecated is not broken, so the
  `oklch` branch now **gamut-maps** instead of clipping per channel — clipping measured as a
  **50.6° hue error on pure blue** (`#A02000` where `#734F00` was asked for), and the
  nearest dye followed it there. `lch` and `hsl` still clip: CIELAB has no in-package gamut
  mapper and adding one to a deprecated path is not the trade. The same deprecation note is
  now on the seven `DyeService.find*` methods that accept `options.colorSpace`.
...
- **Dead-code gate wired into `lint`** (follow-up 6). `pnpm run lint` now ends in `lint:dead` (`knip --directory ../.. --workspace packages/core`) against the new repo-root `knip.jsonc`, with `includeEntryExports` on: an export in `src/index.ts` that no workspace in the monorepo imports fails the lint unless its specifier carries a `/** @public */` JSDoc tag. 76 such specifiers were tagged in this change — the audit's KEEP list (companion types, `facewearColors`, `LEGACY_FACEWEAR_ITEM_IDS`, `RATIO_BANDS`, `MemoryCacheBackend`, `SUPPORTED_LOCALES`, `resolveLocaleFromPreference`, the `utils` survivors, the API tunables). Comments only — no runtime change. See `CLAUDE.md` → "Dead-code gate (knip)".
