...
- **~40 uncalled class methods + 4 legacy overload arms** (DEAD-034/035, Wave 3f): every listed method was verified to have zero callers outside its own facade wrapper and tests before removal.
  ...
  - **Legacy overload arms removed** (DEAD-035): `DyeSearch.findClosestDye`/`DyeService.findClosestDye` no longer accept a bare `excludeIds: number[]` — only `FindClosestOptions`. `DyeSearch.findDyesWithinDistance`/`DyeService.findDyesWithinDistance` no longer accept a bare `maxDistance: number` + trailing `limit?: number` — only `FindWithinDistanceOptions`. In-core callers migrated: `HarmonyGenerator`'s internal `findClosestNonFacewearDye` helper and `CharacterColorService.findClosestDye`'s delegation to `findClosestDyes`. `findDyesWithinDistance`'s `matchingMethod` default is unchanged (`'rgb'`, kept for backwards compatibility per `DyeSearch.ts`) — a follow-up could reconsider that default now that the legacy numeric arm is gone, but this task only fixed the one stale caller that omitted it (`discord-worker`'s `extractor.ts`, below).
