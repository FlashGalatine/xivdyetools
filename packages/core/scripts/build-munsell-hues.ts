...
function main(): void {
  ...
  // TWO files, on purpose. `munsell-hues.json` is what the wheel imports at
  // runtime and ships in every bundle; the 40 raw anchors are only ever read
  // by the cross-check test, and bundling 40 objects of renotation metadata
  // into the web app's modal chunk to serve one test is not a trade worth
  // making. The generator writes both from one pass, so they cannot disagree.
  const dataDir = resolve(dirname(fileURLToPath(import.meta.url)), '../src/data');
  const outPath = resolve(dataDir, 'munsell-hues.json');
  const anchorsPath = resolve(dataDir, 'munsell-anchors.json');
  const provenance =
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        $comment: `${provenance} TEST DATA ONLY — nothing at runtime imports this file; munsell.test.ts cross-checks the generated table against these anchors.`,
        anchors: withWheel,
      },
      null,
      2
  ...
  console.log(`Wrote ${table.length} table rows to ${outPath}`);
  console.log(`Wrote ${withWheel.length} anchors to ${anchorsPath}`);
