...
- **Fixed a pre-existing leak in the same recursion, found reviewing this release (S10-R8): a value aliased from two keys was redacted only at its FIRST reference.** The cycle guard that makes recursive redaction safe was a *global* "seen anywhere" set, so `{ a: shared, b: shared }` (the same object or array referenced from two keys) redacted `a` and returned `b` verbatim — this is FINDING-025's own headline example (`{ tokens: [...] }`) leaking whenever the array happened to be referenced twice. The guard is now an ANCESTOR set: a node is tracked only while it's on the current recursion path and is un-tracked once that branch finishes, so a genuine cycle is still caught (never an infinite loop, never a throw) but an aliased, non-cyclic value is redacted independently at every reference. This bug predates this release (it's the same guard BUG-024 introduced in the 2026-07-18 audit) — it isn't something FINDING-025 introduced — but it lived in the method this release rewrote and is exactly the class of leak this audit is about, so it is fixed here rather than filed separately.
- **Added a total node-visit budget (`MAX_REDACT_NODES` = 5000) as a companion to the fix above.** The old global seen-set had a side effect worth naming: it also deduplicated a heavily-shared (but acyclic) structure, capping total work at one visit per distinct node. Removing that global dedup for correctness means a pathological structure — the same child aliased from both branches at every level of a deep chain — is now walked once per path to it, which is exponential in the chain's depth. The budget bounds this: once exhausted, the (rare, adversarial) remainder of the structure is left un-redacted-further rather than the log call hanging. An ordinary log context (tens to low hundreds of fields) never comes close to the budget.
- **Still not covered:**
