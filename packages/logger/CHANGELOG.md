...
structurally live, most reached in-repo via a subpath (`/browser`, `/worker`, `/library`) rather
than the barrel knip is checking. The `LogLevel`/`LogContext`/`LogEntry`/`Logger`/`LoggerConfig`/
...
- Removed the internal `DEFAULT_REDACT_FIELDS` alias for `CORE_REDACT_FIELDS` in `constants.ts`
  (knip's one duplicate-export finding here). Not a published API change — no barrel in this
  package ever re-exported `constants.ts`, so nothing outside the package could import the alias.
  `core/base-logger.ts`'s two internal uses now reference `CORE_REDACT_FIELDS` directly.
...
- **Still not covered:**
  ...
  - A detected CYCLE's back-edge still serialises one layer of the ORIGINAL, unredacted object before `JSON.stringify`'s own circularity check catches it — `{ o }` where `o.self = o` logs as `{"o":{"name":"o","password":"[REDACTED]","self":{"name":"o","password":"hunter2","self":"[Circular]"}}}`: the primary occurrence is redacted, but the back-edge (`self`) points at the raw original rather than the redacted copy, so its own fields serialise once, raw, before THAT reference is (correctly) recognised as circular and stopped. Pre-existing since the cycle guard was introduced (2026-07-18 audit) and unchanged by this release. This is a **known residual of the current check ordering** (`ancestors` checked before `memo`, `memo` populated only on the way out) — not something proven impossible to close. A different ordering could plausibly make the back-edge resolve to the redacted copy instead of the raw original, but that is a deliberate design change with its own trade-offs (reviewed separately, not made here). Flagged for anyone relying on cyclic context objects.
