...
- `HarmonyCardOptions.wheelLabel` — an optional string the harmony card prints under
  the harmony type when the caller selected a non-default color wheel (`@xivdyetools/core`
...
- **Every color interpolated into a `fill=` / `stroke=` attribute goes through `escapeXml`**
...
`SwatchCardOptions.charName` (required) is renamed to `title`: a neutral card title (localized "Character swatch"), never the character's name or the attachment filename — players routinely use their real name as a Ktisis nickname or `.chara` export filename. Breaking for any external TypeScript consumer passing `charName`. `packages/bot-logic` (3.0.0) is the only in-repo consumer and now passes the localized `card.swatchTitle` key instead of the parsed name.

| Removed | Replacement |
|---|---|
| `SwatchCardOptions.charName: string` | `SwatchCardOptions.title: string` — pass a neutral label, never the character's name |
...
- `generatePaletteGrid(options)` — now the 14K ramp: `band: PaletteBandEntry[]` (every extracted color at its real share) + `rows: PaletteRowEntry[]` (top five by share, source→dye pair); the `width`/column options are gone. New `bandSlices()` helper (7 px floor per slice, remainder taken off the largest).
...
- **New generators** (one module per card, each naming its confirmed design frame in its docblock; all take `theme` and `lang`; heights grow with content and stop at the ceiling):
  - `generateHarmonyCard` (11A, `harmony-card.ts`) — base row + per-slot ideal-vs-found swatch pairs with the angle the math asked for, harmony-calibrated tier words, the matching method printed wherever a tier appears, weakest-slot verdict, four-slot cap with the R1 tail strip. Types `HarmonyCardSlot`, `HarmonyCardLabels`, `HarmonyCardOptions`.
...
- `generateDyeInfoCard` is the 11B sheet: dye-colored 78 px band, two-column numeric grid, SRC row carrying the vendor price, MKT row naming the consolidated market item, and a nearest-dyes strip with match-band tier bars and "+N more".
- `generateRandomDyesGrid` is the 11B table: header row, 52 px rows, stain column, count clamped to five.
- `generatePaletteGrid` shows all extracted colors at their real share in the band; only the top five get rows, and the tenth color can no longer silently disappear.
...
- `preset-swatch.ts` defects fixed in place (the frame redesign of `/preset` is deferred pending the command's survival): centered header strings are measured, the 60-character description cut and `Math.floor(width / 7)` name cut become pixel budgets, and the palette follows the new `THEME`.
