...
- **OPT-005**: `cleanupOldEntries()` now uses in-place `splice()` instead of `filter()` + `set()`, leveraging chronological timestamp ordering to avoid creating new arrays
- **OPT-005**: `pruneOldestEntries()` now reads `timestamps[length - 1]` instead of `Math.max(...timestamps)`, eliminating O(n) spread and preventing potential stack overflow on large arrays
...
- Delete orphaned `src/backends/index.ts` barrel file — never imported by main barrel or `package.json` exports (DEAD-073)
- Remove duplicate `UpstashRateLimiterOptions` interface from `src/backends/upstash.ts` — now imports the canonical definition from `types.ts` (DEAD-074)
