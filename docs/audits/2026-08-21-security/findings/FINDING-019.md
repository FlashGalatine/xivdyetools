...
## Status
**FIXED 2026-08-21** — `@xivdyetools/bot-logic` 2.1.0 helpers; adopted in moderation-worker 1.5.0, discord-worker 5.0.0, stoat-worker 0.2.2, presets-api 2.1.0 validation (bullets below).
...
- presets-api 2.1.0: name/description reject C0 (description keeps tab/newline), DEL, C1, zero-width, bidi-control, U+FEFF, U+2028/9 (ZWJ allowed only between two emoji); tags must match a Unicode letter/number charset with space, `_`, `'`, `’`, `-` (no markdown/URLs); same `VALIDATION_ERROR` envelope.
- discord-worker 5.0.0: `sanitizePresetName`/`Description` delegate to `sanitizeEmbedText` (local caps kept); every builder in `utils/discord-api.ts` (follow-up, edit, message, multipart `payload_json`) sends `allowed_mentions: none` unless overridden; `/preset` list/show/submit/edit/favorites + duplicate echoes + submission-log embeds, webhook author/tags, `/dye search` query, `/swatch` public error embed, `/budget` echoes all sanitised (raw text still goes to the SVG layer, which XML-escapes).
