...
| 4 — corrections + guardrails (DEAD-018, 025, 026, knip gate, i18n orphan gate) | **DONE 2026-08-16** | see `git log` (the commit after `71fba5c`) | DEAD-025: `manifest.json` shortcuts → real routes, `share_target` dropped; `sitemap.xml` regenerated from `ROUTES` (root + 9 tools); `browserconfig.xml` tile was restored in Wave 1. New `src/__tests__/public-metadata.test.ts` pins sitemap/manifest/browserconfig/index.html icon refs to `ROUTES` and to files that exist under `public/`. DEAD-018: 26 `console.info` → `logger.info` (auth-service, extractor, market-board, preset-submission-form, pricing-mixin, api-service-wrapper, main); ESLint `no-console` allow list narrowed to `warn`/`error`. DEAD-026: `analyze-unused-keys.js` rewritten as a real orphan detector (literal + template-prefix + `${…}.suffix` rules), exported with a `.d.ts`, wired as `pnpm run i18n:unused` **and** as a vitest gate (`src/__tests__/i18n-orphans.test.ts`); it immediately caught one more orphan (`palette.savePalette`, the DEAD-030 cascade) → pruned from all six locales (1,053 keys now). knip 6.32.2 added at the workspace root, `apps/web-app/knip.jsonc` checked in (rationale for each ignore inline), `lint` = `eslint src && knip`, `lint:dead` = `knip`, and a package-level `turbo.json` so the lint cache key covers what knip reads; knip runs silent (residual duplicate exports resolved: `ICON_TOOL_DYE_MIXER` alias, `plugin` named export). One test mock (`market-board.test.ts`) needed `info` on its partial logger stub. |
