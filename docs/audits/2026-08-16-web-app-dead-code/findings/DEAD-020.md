...
## Execution note (Wave 3, 2026-08-16)
**Correction to the premise:** the shared components that carry `.number`, `.dye-swatch`, `.empty-state*` and `.component-error-*` also mount in **light-DOM modals** (`preset-edit-form` → `dye-selector` → `dye-grid` → `dye-card-renderer`; any `BaseComponent`'s error boundary), where the page copies DO apply. So those page copies were **kept**, and the rules that must reach both scopes now live in ONE file — `src/styles/tool-content.css` (`@import`ed by `tailwind.css` for the page, and `?inline`-imported by `v4-layout.ts` into the injected `#v5-results-grid-style` for the shell's shadow root). Only rules whose consumers are tools-only (`.extractor-*`, `.section-*`/`.vision-*`/`.contrast-table*`/`.pairwise*`, `.harmony-results-container`, `.comparison-cards-container`, the Tailwind-compat block) were deleted, plus `.harmony-*` (its component was never constructed — DEAD-030). Verified with 18 before/after screenshots (17 pixel-identical; the one 0.18 % diff is `.number` now rendering in Fragment Mono/tabular on the swatch results, as intended).
