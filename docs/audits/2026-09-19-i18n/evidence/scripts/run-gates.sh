#!/usr/bin/env bash
# i18n audit 2026-09-19 — gate + sweep runner. Run from the worktree root.
# Writes one file per gate to ../ (evidence/). Never fails the whole run on one red gate.
set -u
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"      # docs/audits/2026-09-19-i18n
EV="$ROOT/evidence"
...
echo "== 12. audit-local python sweeps =="
SC="$EV/scripts"
...
echo "== 13. innerHTML template sites + hardcoded-sentence grep (tracked files only) =="
...
for u in apps/discord-worker apps/og-worker apps/moderation-worker apps/presets-api apps/api-worker apps/oauth packages/bot-logic packages/svg packages/core; do
  n="$(echo "$u" | tr '/' '-')"
