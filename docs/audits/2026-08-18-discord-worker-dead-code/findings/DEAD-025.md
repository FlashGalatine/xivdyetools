...
Per-type disposition: **swapped** for the shared import — `AuthUser`, `AuthProvider`, `AuthResponse`, `JWTPayload`, `PresetStatus`, `CommunityPreset`, `PresetFilters`, `PresetListResponse`, `PresetSubmission`, `PresetEditRequest`, `PresetSortOption`. **Dropped** — `PrimaryCharacter`: web-app deleted its local copy and imports nothing in its place; the type flows structurally through the swapped-in `AuthUser`, so no explicit import of `PrimaryCharacter` is needed. **Kept local** — `VoteResponse`: presets-api returns `{success: true, already_voted: true, new_vote_count}` on HTTP 409 (`apps/presets-api/src/handlers/votes.ts` `addVote()`), but the web-app client deliberately synthesises `success: false` for that case so its "already voted" UI branch fires instead of the success branch — a shape the shared `VoteErrorResponse` (`success: false; error: string` only) can't hold.
