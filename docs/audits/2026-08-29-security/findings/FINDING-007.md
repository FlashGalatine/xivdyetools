...
## Status
FIXED 2026-08-30 6c14889f, d28f76a4 (discord-worker 5.1.0: six native `[[ratelimits]]` tiers `RL_5`…`RL_70` per env — production `namespace_id`s 1041–1046, beta 1051–1056 — through worker-kit's `CloudflareRateLimiter`; Upstash removed from the worker (config type, `Env`, wrangler, docs); KV is the fallback only when no tier is bound (warns once per isolate through the request logger); fail-open on a binding error is reported per request from `backendError`; PRIVACY_POLICY.md §2/§5/§8 rewritten in f5d5f596; fix wave 4d734c8c warns once per isolate naming any missing tiers (worker-kit would otherwise route to the next larger tier silently) and fe86a881 makes production refuse requests without all six. **Deploy needs — DONE 2026-08-31:** both `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` were deleted from discord-worker production; the dashboard's runtime variable list now ends at `SUBMISSION_LOG_CHANNEL_ID`, where `UPSTASH_*` would sort. Confirmed afterwards that `https://bot.xivdyetools.app/health` answers 200 — a real check rather than a formality, since production refuses every request, `/health` included, while any `RL_*` binding is missing, so a healthy response proves all six native tiers are bound and that removing the Upstash credentials left nothing dangling.)
