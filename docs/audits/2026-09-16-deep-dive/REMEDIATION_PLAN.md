...
**Ends with:** `pnpm turbo run build test --filter=@xivdyetools/bot-logic` → bump `packages/bot-logic/package.json` (4.2.0 → 4.3.0, minor) → merge to `main` → Actions → **"Publish Packages to npm"** → `@xivdyetools/bot-logic`.
