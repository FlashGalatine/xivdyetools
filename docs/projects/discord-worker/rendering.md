...
- The subsets cover the dye names **and** every bot-UI string from `@xivdyetools/bot-logic/i18n`, plus the localized market-item names in `packages/core/src/config/consolidated-ids.ts`. `scripts/subset-cjk-fonts.py` scans those locale sources and cuts each Noto Sans face down to only the codepoints they use — SC also carries every ja/zh ideograph since it is the terminal fallback in every locale's font stack, JP is scoped to `ja` text only, and KR is scoped to Hangul + ASCII (Korean strings use no CJK ideographs). The codepoint count shifts with every locale-text edit, so no fixed number stays accurate here — re-run the script to print the current totals and each subset's size/glyph count.
