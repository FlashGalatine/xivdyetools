...
| 2026-08-29 | moderation bot token (`DISCORD_TOKEN` on moderation-worker, `MODERATION_BOT_TOKEN` on discord-worker, GitHub `MODERATION_DISCORD_TOKEN`) | maintainer | reset in the Developer Portal so the new *Register Discord commands* CI step could get a value it never had; the GitHub secret was set the same day. **Pending until ticked here:** the two production worker secrets (`--env production`, per §4) — until then moderation channel posts / embed edits answer 401 |
